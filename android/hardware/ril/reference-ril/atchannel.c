...
//add by alex
extern sig_atomic_t s_data_fd;                   
extern pthread_mutex_t s_power_mutex;            
extern pthread_cond_t s_power_cond;
extern sig_atomic_t modemPowerState;
...
static char s_data_ATBuffer[MAX_AT_RESPONSE+1];   //add by xue
static char *s_data_ATBufferCur = s_data_ATBuffer;     //add by xue

static int s_ackPowerIoctl; /* true if TTY has android byte-count
                                handshake for low power*/
static int s_readCount = 0;

static int s_data_readCount = 0;      //add by xue
#if AT_DEBUG
...
const char *readline_data()
{
    ...
    /* this is a little odd. I use *s_data_ATBufferCur == 0 to
     * mean "buffer consumed completely". If it points to a character, than
     * the buffer continues until a \0
     */
    if (*s_data_ATBufferCur == '\0') {
        /* empty buffer */
        s_data_ATBufferCur = s_data_ATBuffer;
        *s_data_ATBufferCur = '\0';
        p_read = s_data_ATBuffer;
    } else {   /* *s_data_ATBufferCur != '\0' */
        /* there's data in the buffer from the last read */

        // skip over leading newlines
        while (*s_data_ATBufferCur == '\r' || *s_data_ATBufferCur == '\n')
            s_data_ATBufferCur++;

        p_eol = findNextEOL(s_data_ATBufferCur);

        if (p_eol == NULL) {
            /* a partial line. move it up and prepare to read more */
            size_t len;

            len = strlen(s_data_ATBufferCur);

            memmove(s_data_ATBuffer, s_data_ATBufferCur, len + 1);
            p_read = s_data_ATBuffer + len;
            s_data_ATBufferCur = s_data_ATBuffer;
        }
        /* Otherwise, (p_eol !- NULL) there is a complete line  */
        /* that will be returned the while () loop below        */
    ...
    while (p_eol == NULL) {
        if (0 == MAX_AT_RESPONSE - (p_read - s_data_ATBuffer)) {
            LOGE("ERROR: Input line exceeded buffer\n");
            /* ditch buffer and start over again */
            s_data_ATBufferCur = s_data_ATBuffer;
            *s_data_ATBufferCur = '\0';
        ...
        do {
            count = read(s_data_fd, p_read,
                            MAX_AT_RESPONSE - (p_read - s_data_ATBuffer));
        } while (count < 0 && errno == EINTR);

        if (count > 0) {
           // AT_DUMP( "<< ", p_read, count );
            LOGE( "DATA<< %s , %d", p_read, count );
            s_data_readCount += count;
    ...
    ret = s_data_ATBufferCur;
    *p_eol = '\0';
    s_data_ATBufferCur = p_eol + 1; /* this will always be <= p_read,    */
                              /* and there will be a \0 at *p_read */
...
static void checkAirPlaneMode()
{
   pthread_mutex_lock(&s_power_mutex);
   while(!modemPowerState)
   {
      pthread_cond_wait(&s_power_cond, &s_power_mutex);
   }
 ...
 * have buffered stdio.
 */
static int writeline_data (const char *s)
{
    size_t cur = 0;
    size_t len = strlen(s);
    ssize_t written = 0;
    ...
    while (cur < len) {
        ...
        LOGD("AT DATA writed fd(%d) %d char.", fd, written);
    ...
    do {
        ...
        LOGD("AT DATA writed  2 fd(%d) %d char.", fd, written);
    if (written < 0) {
        return AT_ERROR_GENERIC;
    }
 ...
 * timeoutMsec == 0 means infinite timeout
 */
int at_send_data_command_full_nolock (const char *command, ATCommandType type,
                    const char *responsePrefix, const char *smspdu,
                    long long timeoutMsec, ATResponse **pp_outResponse)
{
    int err = 0;
    ...
    err = writeline_data(command);
    if(err < 0) {
       LOGE("xue, %s, write data line error!\n", __FUNCTION__);
       return -1;
    }
   ...
   // return 0;
    ...
    {
       err = 0;
       LOGE("xue, %s, setup data link successful.\n", __FUNCTION__);
    }
    else
    {
       LOGE("xue, %s, setup data link failed.\n", __FUNCTION__);
       err = -1;
    }
