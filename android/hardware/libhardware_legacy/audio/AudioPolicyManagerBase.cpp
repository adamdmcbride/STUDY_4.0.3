                                                  ...
                                                  const char *device_address)
{
    ...
    ZJFLOGD("function in, device is 0x%x, state is %d.", device, state);
    ...
    if (AudioSystem::isOutputDevice(device)) {
        ...
        case AudioSystem::DEVICE_STATE_AVAILABLE:
            ZJFLOGD("case AudioSystem::DEVICE_STATE_AVAILABLE, device is 0x%x", device);
...
#endif
            {
                ...
                } else if (AudioSystem::isWireHeadsetDevice(device)) {
                    status_t status = handleWireHPConnection();
                    if (status != NO_ERROR) {
                        mAvailableOutputDevices &= ~device;
                        return status;
                    }
                } else if (AudioSystem::isHDMIAudioDevice(device)) {
                    status_t status = handleHDMIAudioConnection();
                    if (status != NO_ERROR) {
                        mAvailableOutputDevices &= ~device;
                        return status;
                    }
        ...
        case AudioSystem::DEVICE_STATE_UNAVAILABLE: {
            ...
            
            {
                if (AudioSystem::isBluetoothScoDevice(device)) {
                    ...
                    status_t status = handleWireHPDisconnection();
                    if (status != NO_ERROR) {
                        mAvailableOutputDevices &= ~device;
                        return status;
                    }
                } else if (AudioSystem::isHDMIAudioDevice(device)) {
                    status_t status = handleHDMIAudioDisconnection();
                    if (status != NO_ERROR) {
                        mAvailableOutputDevices &= ~device;
                        return status;
                    }
    ...
    ZJFLOGD("function out for BAD_VALUE.");
    return BAD_VALUE;
}
...
/*************************************************************************/
status_t AudioPolicyManagerBase::handleWireHPConnection()
{
    ZJFLOGD("function in.");
    if (mHardwareOutput) {
        ZJFLOGD("will setParameters for hpconnection");
        AudioParameter param;
        param.addInt(String8("headphone_plugin"), 1); 
...
status_t AudioPolicyManagerBase::handleWireHPDisconnection()
{
    ZJFLOGD("function in.");
    if (mHardwareOutput) {
        // add A2DP output descriptor
        AudioParameter param;
        param.addInt(String8("headphone_plugin"), 0);
        ZJFLOGD("will setParameters for hpdisconnection");
        mpClientInterface->setParameters(mHardwareOutput, param.toString());
    }
    AudioOutputDescriptor *hwOutputDesc = mOutputs.valueFor(mHardwareOutput);
...
status_t AudioPolicyManagerBase::handleHDMIAudioConnection()
{
    ZJFLOGD("function in.");
    if (mHardwareOutput) {
        ZJFLOGD("will setParameters for hdmiconnection");
        AudioParameter param;
        param.addInt(String8("hdmi_audio_plugin"), 1); 
        mpClientInterface->setParameters(mHardwareOutput, param.toString());
    }
    AudioOutputDescriptor *hwOutputDesc = mOutputs.valueFor(mHardwareOutput);
...
status_t AudioPolicyManagerBase::handleHDMIAudioDisconnection()
{
    ZJFLOGD("function in.");
    if (mHardwareOutput) {
        // add A2DP output descriptor
        AudioParameter param;
        param.addInt(String8("hdmi_audio_plugin"), 0);
        ZJFLOGD("will setParameters for hdmidisconnection");
        mpClientInterface->setParameters(mHardwareOutput, param.toString());
    }
    AudioOutputDescriptor *hwOutputDesc = mOutputs.valueFor(mHardwareOutput);
