...
struct cpuidle_driver ns115_idle = {
	.name = "ns115_idle",
	.owner = THIS_MODULE,
};
...
struct ns115_suspend_platform_data
{
	unsigned int cpu_timer;
};

extern unsigned int ns115_lp2_allowed(void);
extern unsigned int ns115_suspend_lp2(unsigned int);
static int ns115_idle_enter_wfi(struct cpuidle_device *dev,
	struct cpuidle_state *state);

static int ns115_idle_enter_lp2(struct cpuidle_device *dev,
	struct cpuidle_state *state)
{
	ktime_t enter;
	...
	if (!lp2_supported || request <= idle_us || (!ts->tick_stopped) ||
		system_is_suspending || (!ns115_lp2_allowed())) {
		dev->last_state = &dev->states[0];
		return ns115_idle_enter_wfi(dev, &dev->states[0]);
	}

	local_irq_disable();
	enter = ktime_get();
	request -= state->exit_latency;
	us = ns115_suspend_lp2((unsigned int)max_t(s64, 200, request));
...
static int ns115_idle_lp2_allowed(struct notifier_block *nfb,
...
static int ns115_cpuidle_notifier(struct notifier_block *nfb,
...
static int ns115_idle_enter(unsigned int cpu)
{
	...
	state->enter = ns115_idle_enter_wfi;
	...
	if (cpu == 0) {
		...
		state->enter = ns115_idle_enter_lp2;
...
#ifdef CPU_POWER_GATE
	lp2_supported = (num_online_cpus()==1);
	hotcpu_notifier(ns115_idle_lp2_allowed, 0);
	pm_notifier(ns115_cpuidle_notifier, 0);
#endif

	ret = cpuidle_register_driver(&ns115_idle);

	pr_debug("ns115_cpuidle_init ret = %08X\n", ret);

	if (ret)
		return ret;

	for_each_possible_cpu(cpu) {
		if (ns115_idle_enter(cpu))
...
static void __exit ns115_cpuidle_exit(void)
{
	cpuidle_unregister_driver(&ns115_idle);
}


module_init(ns115_cpuidle_init);
module_exit(ns115_cpuidle_exit);
