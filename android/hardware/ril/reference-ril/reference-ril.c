...
#define PPP_TTY_PATH "ppp0"            //add by xue
#define DATABITS       CS8   
#define BAUD          B115200
#define STOPBITS        0    
#define PARITYON        0    
...
static void getDataCallListWhenSetup(void *param);           //add by xue

//add by xue
extern int at_send_data_command_full_nolock (const char *command, ATCommandType type,
...
//add by alex
pthread_mutex_t s_power_mutex = PTHREAD_MUTEX_INITIALIZER;            
pthread_cond_t s_power_cond = PTHREAD_COND_INITIALIZER;
sig_atomic_t modemPowerState = 1;
...
static const char * s_data_device_path = "/dev/ttyACM0";             //add by xue
...
static sig_atomic_t network_stat = 0;          //add by xue
sig_atomic_t s_data_fd = -1;                   //add by xue
static sig_atomic_t deactive_ppp_flag = -1;
static RIL_Token tmp_token = NULL;             //add by xue
static SIM_Status sim_s_f = SIM_ABSENT;        //add by xue
static sig_atomic_t network_type = -1;
...
static void setModemPower(int onOff)
{
#define POWER_HANDLER "/sys/class/gpio/gpio140/value"

   int err = -1;
   int fd_gpio;
   char reset_buff[2] = {'1','0'};

   fd_gpio = open(POWER_HANDLER, O_RDWR);
   if(fd_gpio < 0)
   {
      LOGE("-----POWER_HANDLER open error!!!---ALEX");
      goto done;
   }

   pthread_mutex_lock(&s_power_mutex);

   if(onOff == 0)
   {
      modemPowerState = 0;
      err = write(fd_gpio, &reset_buff[1], 1);      //power off
      if(err < 0)
      {
         LOGE("SET modem power off failed.---ALEX");
         goto done;
      }
         LOGD("SET modem power off successfully.---ALEX");
      close(fd_gpio);
   }
   else
   {
      err = write(fd_gpio, &reset_buff[1], 1);      //power on
      if(err >= 0)
      {
         LOGD("Set modem power on successfully!");
         /*
         struct stat file_stat;
         if(stat(buf, &file_stat) == 0)
         {
            LOGE("ppp0 still exist. sleep 1 second.");
            sleep(1);
         }
         else
         {
            LOGD("ppp0 was destroy successfully.");
            break;
         }
         */
      }
      else
         LOGE("SET modem power on failed.---ALEX");
done:
      modemPowerState = 1;
      pthread_cond_signal(&s_power_cond);
      close(fd_gpio);
   }

   pthread_mutex_unlock(&s_power_mutex);
}
...
static void requestRadioPower(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (onOff == 0 && sState != RADIO_STATE_OFF) {
       static int phoneSetupF = 0;
        err = at_send_command("AT+CFUN=4", &p_response);
       if (err < 0 || p_response->success == 0) goto error;
        setRadioState(RADIO_STATE_OFF);
        if(phoneSetupF++ > 0)                                             //add by alex.  
        {
            setModemPower(0);                                            
        }
    } else if (onOff > 0 && sState == RADIO_STATE_OFF) {
       if(modemPowerState == 0)                                          //add by alex.
       {
          setModemPower(1);                                            
...
//add by xue
static void getDataCallListWhenSetup(void *param)
{
...
static void requestOrSendDataCallList(RIL_Token *t)
{
    ...
    if(NULL != t && 1 == n && (response[0].active == 0 || strStartsWith(out, "0.0.0.0")))
    {
       LOGE("get ip failed, try again after 1 s!!");

       if((times--) > 0)
       {
          struct timeval TIMEVAL_1 = { 1, 0 };
          RIL_requestTimedCallback(getDataCallListWhenSetup, t, &TIMEVAL_1);

          return;
       }
       else 
       {
          LOGE("get ip failed 3 times, return error.!!");
          goto error;
       }
    }
...
//    int response[2];
    ...
    memset(&response, 0, sizeof(RIL_SignalStrength_v5));
#if 1
   ...
   // err = at_tok_nextint(&line, &(response[1]));
    ...
    {
       LOGE("-------xue------%s---signal is true-\n", __FUNCTION__);
       response.GW_SignalStrength.signalStrength = 31;
       response.GW_SignalStrength.bitErrorRate = 20;
       response.CDMA_SignalStrength.dbm = 0;      
       response.CDMA_SignalStrength.ecio = 0;     
       response.EVDO_SignalStrength.dbm = 0;  
       response.EVDO_SignalStrength.ecio = 0; 
...
static int libConvertActToRilState(int AcT)      
{                                                              
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    switch (commas) {
        case 0: /* +CREG: <stat> */
            ...
            response[1] = -1;
            response[2] = -1;
           network_stat = response[0];       //add by xue
        break;
    ...
    if(-1 != network_type)
    {
       response[3] = libConvertActToRilState(network_type);
    }
    else
    {
       response[3] = 0;
    ...
    if(sim_s_f != SIM_READY)
    {
       response[0] = 12; // workaround for android
    }
...
#define PPPD_PID "net.ppp.pid"
#define PPPD_PID_FILE "/data/pppd.pid"
...
static void requestDeactiveDataCall(void *data, size_t datalen, RIL_Token t)
{
   ...
   LOGE("enter -----------------------requestDeactiveDataCall-----------");

 //  err = at_send_data_command_full_nolock("AT+CGACT=0,1", 0, NULL, NULL, 0, NULL);
   ...
   char  propValue[PROP_VALUE_MAX];
   pid_t pid = -1;
   if (__system_property_get(PPPD_PID, propValue) != 0) 
   {
      if((pid = atoi(propValue)) > 0)
      {
         err = kill(pid, SIGKILL);
         if(err < 0)
         {
            LOGE("1. kill process of the pppd failed !!");
         }
         else
         {
            LOGD("1. kill process of the pppd successful !!!");
         }
      }
   }
   else
   {
      LOGE(" read pppd pid property failed!!!");
      // Kill PPPD if it exists
      FILE* fp = fopen(PPPD_PID_FILE, "r");
      if ( fp )
      {
         fscanf(fp, "%d", &pid);
         if ( pid != -1 )
         {
            ...
            if(err < 0)
            {
               LOGE("2. kill process of the pppd failed !!");
            }
            else
            {
               LOGD("2. kill process of the pppd successful !!!");
            }
         }
         else
            LOGE("pppd pid format is error!!!");
         fclose(fp);
      }
      else
      {
         LOGE(" open pppd pid file failed!!!");
      }
   }
   deactive_ppp_flag = 1;
...
extern const char *readline_data();

static void enablePPP()
{
   int ret = -1;
   LOGE("ready to enable PPP");

   ret = system("echo \'ATDT*99#\r\' > /dev/ttyACM0");

   if(-1 == ret)
   {
      LOGE("---xue---------system oper failed!\n");
   }
   else if(127 == ret)
   {
      LOGE("---xue---------system return 127.errno:%d", errno);
   }
   else if(0 == ret)
   {
      LOGE("--xue--ret:%d--------system send ATD*99# successful!\n", ret);
   //   const char *line = readline_data();
   //   LOGE("AT < %s", line);

      ret = system("/system/bin/pppd ttyACM0 460800 usepeerdns noipdefault defaultroute 0.0.0.0:0.0.0.0");
      ...
      {
         LOGE("--xue--ret:%d--------system oper successful!\n", ret);
      }
   }
...
static void requestSetupDataCall(void *data, size_t datalen, RIL_Token t)
{
    ...
    if(-1 != s_data_fd) 
    {
       LOGE("----------------data state is connected now!!--close it-----------------------");
       close(s_data_fd);
       s_data_fd = -1;
    ...
    int times = 3;
    sprintf(buf, "/sys/class/net/%s", PPP_TTY_PATH);
    while(times--)
    {
       if(stat(buf, &file_stat) == 0)
       {
    ...
    if(times < 0)
    {
          LOGE("ppp0 still exist. return error!!!");
       goto error;
    }

    {
       ...
       if ( fd >= 0 && !memcmp( s_data_device_path, "/dev/ttyA", 9 ) ) 
       {
#if 1
          fcntl(fd, F_SETFL, 0);
          ...
          LOGD("-----------open data channel success!---------------fd(%d)-----", fd);
          /*
       ...
       {
       //   perror ("opening AT DATA interface. retrying...");
          LOGE("---xue func:%s, can not open AT DATA channel: %s\n", __FUNCTION__,  s_data_device_path);

          goto error;
          /* never returns */
       }

       LOGE("xue--------------open data fd OK!\n");
    ...
    {
       err = at_send_command("AT+CGACT=0,1", &p_response);
        if(-1 == err || 0 == p_response->success)
        {
           LOGE("----------------disable data connect failed!!--------------");
           goto error;
        }
...
*/
        err = at_send_data_command_full_nolock("ATE0", 0, NULL, NULL, 0, NULL);

//        err = at_send_data_command_full_nolock("AT+CGACT=0,1", 0, NULL, NULL, 0, NULL);

        sleep(1);

        err = at_send_data_command_full_nolock("AT", 0, NULL, NULL, 0, NULL);
        if (err < 0) {
            goto error;
        }
    ...
    //    err = at_send_data_command_full_nolock("ATDT*99#\r\n", 0, NULL, NULL, 0, NULL);
     //   if (err < 0) {
    //        goto error;
    //    }
    }
 ...
 //   RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
    struct timeval TIMEVAL_4 = { 4, 0 };
    tmp_token = t;
    RIL_requestTimedCallback(getDataCallListWhenSetup, &tmp_token, &TIMEVAL_4);
    //requestOrSendDataCallList(&t);
    ...
    for (;;) {
        ...
        while  (fd < 0) {
            ...
            if (s_port > 0) {
                ...
                if (fd < 0) {
                   LOGE("opening AT channel failed. retrying...\n");
                   sleep(10);
                   continue;
                   /* never returns */
                }
                ...
                tcsetattr(fd, TCSANOW, &newtio); 
#else
                if ( fd >= 0 && !memcmp( s_device_path, "/dev/ttyA", 9 ) ) {
                    /* disable echo on serial ports */
                    struct termios  ios;
                    tcgetattr( fd, &ios );
                    ios.c_lflag = 0;  /* disable ECHO, ICANON, etc... */
