/* wangweilin modify 
 * v0.7 2012.1.14 remove warning
 * v0.6 2012.1.14 add ifdef for suspend and early_suspend
 * v0.5 2012.1.12 add early_suspend and kernel3.0 porting
 * v0.4 2011.7. 2 backup
 * v0.3 2011.7. 1 output data, i2c num is 0x18
 * v0.2 2011.7. 1 test
 * v0.1 2011.6.30 init
 */
...
#define VERSION "0.3"	//VERSION

#define TR0(fmt, args...) printk(KERN_CRIT "gsensor_kxud9: " fmt, ##args)
#define TR1(fmt, args...) 
...
static int kxud9_get_acceleration_data(int *xyz)
{
	...
	//TR1("x:%5d y:%5d z:%5d-------- x:%5d y:%5d z:%5d\n", xyz[0], xyz[1], xyz[2], hw_d[0], hw_d[1], hw_d[2]);
...
{
#ifdef	CONFIG_NS2816_HUASHAN_SKYTONE
	input_report_abs(ud9->input_dev, ABS_X, -xyz[0]);
	input_report_abs(ud9->input_dev, ABS_Y, -xyz[1]);
#endif
#ifdef CONFIG_NS2816_HUASHAN_TONGFANG
	input_report_abs(ud9->input_dev, ABS_X, -xyz[1]);
        input_report_abs(ud9->input_dev, ABS_Y, xyz[0]);
#endif
#ifdef CONFIG_MACH_NS2816_YANGCHENG
	input_report_abs(ud9->input_dev, ABS_X, xyz[1]);
	input_report_abs(ud9->input_dev, ABS_Y, -xyz[0]);
#endif
#ifdef CONFIG_NS2816_HUASHAN_NETXEON
	input_report_abs(ud9->input_dev, ABS_X, xyz[0]);
	input_report_abs(ud9->input_dev, ABS_Y, -xyz[1]);
#else
	input_report_abs(ud9->input_dev, ABS_X, xyz[0]);
...
static void kxud9_early_suspend(struct early_suspend * h)
{
	TR0("early_suspend\n");
	kxud9_disable();
}

static void kxud9_early_resume(struct early_suspend * h)
{
	TR0("early_resume\n");
	kxud9_enable();
}
static struct early_suspend es = {
        ...
        .suspend = kxud9_early_suspend,
        .resume = kxud9_early_resume,
};
#endif
...
MODULE_DESCRIPTION("nufront kxud9 wangweilin modify");
MODULE_AUTHOR("weilin.wang@nufront.com");
MODULE_LICENSE("GPL");
