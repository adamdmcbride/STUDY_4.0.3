...
status_t ColorConverter::convert(
    ...
    /* a@nufront begin */
    void* swap_src;
    if (mSrcFormat == OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar) {
        // get virtual address from data buffer;
        swap_src = (void*)(*(((int*)srcBits) + 1));
    } else {
        swap_src = (void*)srcBits;
    }
    ...
    switch (mSrcFormat) {
        ...
        /*a@nufront start*/
        case OMX_COLOR_Format16bitRGB565:
            err = convert16bitRGB565(src, dst);
            break;
        /*a@nufront end*/
...
/*a@nufront start*/
status_t ColorConverter::convert16bitRGB565(
        const BitmapParams &src, const BitmapParams &dst) {
    void *dstBits = (void*)dst.mBits;
    void *srcBits = (void*)(*((uint32_t*)src.mBits + 1));

    if((dstBits) && (!srcBits))
    {
        //LOGD("src buffer error, display black frame, dstBits=%p, srcBits=%p\n", dstBits, srcBits);
        memset(dstBits, 0x0, dst.mWidth * dst.mHeight * 2);
        
        return OK;
    }

    memcpy (dstBits, srcBits, dst.mWidth * dst.mHeight * 2);
