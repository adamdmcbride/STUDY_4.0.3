...
public class VideoCamera extends ActivityBase
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            String externalStoragePath = Environment.getExternalStorageDirectory().toString().trim();
            Uri dataUri = intent.getData();
            String dataPath = dataUri.getPath().trim();
            if ((dataPath != null) && (!dataPath.equals(externalStoragePath)))
            {
                return;
            }
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                ...
                Log.e(TAG,"MyBroadcastReceiver.onReceive,case Intent.ACTION_MEDIA_EJECT, call stopVideoRecording()");
                stopVideoRecording();
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (mIsVideoCaptureIntent) {
            ...
            //if current is front camera, remove panorama mode
            CameraInfo info = CameraHolder.instance().getCameraInfo()[mCameraId];
            if (info.facing == CameraInfo.CAMERA_FACING_FRONT){
                ViewGroup modeSelectLayout = (ViewGroup)findViewById(R.id.mode_selection);
                RotateImageView panoramaView = (RotateImageView)findViewById(R.id.mode_panorama);
                modeSelectLayout.removeView(panoramaView);
            }
    ...
    public void onReviewCancelClicked(View v) {
        Log.e(TAG,"onReviewCancelClicked, call stopVideoRecording()");
        stopVideoRecording();
        doReturnToCaller(false);
    }

    private void onStopVideoRecording(boolean valid) {
        Log.e(TAG,"onStopVideoRecording, call stopVideoRecording()");
        stopVideoRecording();
        if (mIsVideoCaptureIntent) {
            if (mQuickCapture) {
    ...
    public void onShutterButtonClick() {
        ...
        if (stop) {
            Log.e(TAG,"onShutterButtonClick, call stopVideoRecording()");
    ...
    private void readVideoPreferences() {
        ...
        Log.v(TAG,"readVideoPreferences,defaultQuality:" + defaultQuality + ",videoQuality:" + videoQuality);
        // Set video quality.
        ...
        if (intent.hasExtra(MediaStore.EXTRA_VIDEO_QUALITY)) {
            ...
            if (extraVideoQuality > 0) {
                ...
                Log.v(TAG,"Set video quality:CamcorderProfile.QUALITY_HIGH");
            } else {  // 0 is mms.
                quality = CamcorderProfile.QUALITY_LOW;
                Log.v(TAG,"Set video quality:CamcorderProfile.QUALITY_LOW");
            }
        }
        ...
        Log.v(TAG,"Set video mMaxVideoDurationInMs:" + mMaxVideoDurationInMs);

        // Set effect
        mEffectType = CameraSettings.readEffectType(mPreferences);
        if (mEffectType != EffectsRecorder.EFFECT_NONE) {
            Log.v(TAG,"readVideoPreferences, case mEffectType != EffectsRecorder.EFFECT_NONE");
            ...
            if (!intent.hasExtra(MediaStore.EXTRA_VIDEO_QUALITY)) {
                Log.v(TAG,"readVideoPreferences,  quality = CamcorderProfile.QUALITY_480P");
            ...
            // initializeIndicatorControl.
            if (mIndicatorControlContainer != null) {
                Log.v(TAG,"readVideoPreferences, call mIndicatorControlContainer.overrideSettings(480P)");    
                mIndicatorControlContainer.overrideSettings(
                        CameraSettings.KEY_VIDEO_QUALITY,
                        //Integer.toString(CamcorderProfile.QUALITY_480P));
            ...
            if (mIndicatorControlContainer != null) {
                Log.v(TAG,"readVideoPreferences, call mIndicatorControlContainer.overrideSettings(NULL)");    
                mIndicatorControlContainer.overrideSettings(
                        CameraSettings.KEY_VIDEO_QUALITY,
                        null);
            }
        }
        Log.v(TAG,"readVideoPreferences, read time lapse recording interval");
        ...
        Log.v(TAG,"readVideoPreferences, call mProfile = CamcorderProfile.get(mCameraId, quality:" + quality);
        mProfile = CamcorderProfile.get(mCameraId, quality);
        getDesiredPreviewSize();
        //zhuyx--add--[
        /*
        Log.v(TAG,"readVideoPreferences, we change quality to 1007(320*240)");
        mProfile = CamcorderProfile.get(mCameraId, 1007);
        getDesiredPreviewSize();
        */
        //zhuyx--add--]
    ...
    private void getDesiredPreviewSize() {
        ...
        if (mParameters.getSupportedVideoSizes() == null || effectsActive()) {
            Log.v(TAG,"getDesiredPreviewSize(), get from profile");
            mDesiredPreviewWidth = mProfile.videoFrameWidth;
            mDesiredPreviewHeight = mProfile.videoFrameHeight;
        } else {  // Driver supports separates outputs for preview and video.
            Log.v(TAG,"getDesiredPreviewSize(), get from HAL support");
            ...
            Log.v(TAG,"getDesiredPreviewSize(), get from HAL support, call Util.getOptimalPreviewSize");
            Size optimalSize = Util.getOptimalPreviewSize(this, sizes,
                (double) mProfile.videoFrameWidth / mProfile.videoFrameHeight);
            mDesiredPreviewWidth = optimalSize.width;
            mDesiredPreviewHeight = optimalSize.height;
        }
        Log.v(TAG, "getDesiredPreviewSize(), mDesiredPreviewWidth=" + mDesiredPreviewWidth +
                ". mDesiredPreviewHeight=" + mDesiredPreviewHeight);
        //zhuyx--add--[
        //Log.v(TAG, "getDesiredPreviewSize(), we change it to 320x240/640x480 cause we just support it");
        //mDesiredPreviewWidth = 320;
        //mDesiredPreviewHeight = 240;
    ...
    private void finishRecorderAndCloseCamera() {
        ...
        if (mMediaRecorderRecording) {
            if (mIsVideoCaptureIntent) {
                Log.e(TAG,"finishRecorderAndCloseCamera,case mIsVideoCaptureIntent, call stopVideoRecording()");
                stopVideoRecording();
                showAlert();
            } else {
                Log.e(TAG,"finishRecorderAndCloseCamera,case not mIsVideoCaptureIntent,call stopVideoRecording()");
                stopVideoRecording();
                getThumbnail();
            }
        } else {
            Log.e(TAG,"finishRecorderAndCloseCamera,case not mMediaRecorderRecording,call stopVideoRecording()");
            stopVideoRecording();
        }
        closeCamera();
    }
    ...
    public void onBackPressed() {
        if (mPausing) return;
        if (mMediaRecorderRecording) {
            Log.e(TAG,"onBackPressed,call stopVideoRecording()");
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        ...
        if (mPreviewing && (Util.getDisplayRotation(this) == mDisplayRotation)
            ...
            Log.e(TAG,"surfaceChanged,call stopVideoRecording()");
            stopVideoRecording();
            startPreview();
        }
    }
    ...
    private void initializeRecorder() {
        ...
        Log.v(TAG, "initializeRecorder, call new MediaRecorder(),then call MediaProfiles::getInstance() by JNI to get media_config.xml");
        ...
        Log.v(TAG, "initializeRecorder, setMaxDuration:" + mMaxVideoDurationInMs);
        ...
        Log.v(TAG, "initializeRecorder, setOutputFile:" + mVideoFilename);

        mMediaRecorder.setPreviewDisplay(mSurfaceHolder.getSurface());

        // Set maximum file size.
        Log.v(TAG, "initializeRecorder, mStorageSpace:" + mStorageSpace);
        Log.v(TAG, "initializeRecorder, Storage.LOW_STORAGE_THRESHOLD:" + Storage.LOW_STORAGE_THRESHOLD);
        ...
        Log.v(TAG, "initializeRecorder, setMaxFileSize:" + maxFileSize);
        ...
        Log.v(TAG, "initializeRecorder, setOrientationHint:" + rotation);
...
*/
    ...
    public void onError(MediaRecorder mr, int what, int extra) {
        ...
        if (what == MediaRecorder.MEDIA_RECORDER_ERROR_UNKNOWN) {
            ...
            Log.e(TAG,"onError,case MEDIA_RECORDER_ERROR_UNKNOWN ,call stopVideoRecording()");
            stopVideoRecording();
            updateAndShowStorageHint();
        }
    }
    ...
    public void onInfo(MediaRecorder mr, int what, int extra) {
        if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_DURATION_REACHED) {
            Log.e(TAG,"onError,case MEDIA_RECORDER_INFO_MAX_DURATION_REACHED ,call onStopVideoRecording(true)");
            if (mMediaRecorderRecording) onStopVideoRecording(true);
        } else if (what == MediaRecorder.MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED) {
            Log.e(TAG,"onError,case MEDIA_RECORDER_INFO_MAX_FILESIZE_REACHED ,call onStopVideoRecording(true)");
            if (mMediaRecorderRecording) onStopVideoRecording(true);
    ...
    private void setCameraParameters() {
        mParameters = mCameraDevice.getParameters();
        Log.e(TAG,"setCameraParameters, call setPreviewSize(" + mDesiredPreviewWidth + "x" +mDesiredPreviewHeight + ")");
        mParameters.setPreviewSize(mDesiredPreviewWidth, mDesiredPreviewHeight);
        Log.e(TAG,"setCameraParameters, call setPreviewFrameRate(),mProfile.videoFrameRate:" + mProfile.videoFrameRate);
        ...
        synchronized (mPreferences) {
            ...
            if (mCameraId != cameraId) {
                ...
                if (size.width != mDesiredPreviewWidth
                    ...
                    Log.e(TAG,"onSharedPreferenceChanged,restart the preview if preview size is changed.call startPreview");
                    if (!effectsActive()) {
                        mCameraDevice.stopPreview();
                    } else {
                        mEffectsRecorder.release();
    ...
    private void checkQualityAndStartPreview() {
        Log.e(TAG,"checkQualityAndStartPreview");
        readVideoPreferences();
        showTimeLapseUI(mCaptureTimeLapse);
        Size size = mParameters.getPreviewSize();
        if (size.width != mDesiredPreviewWidth
