...
static void backlight_control(int on, int delayms)
{
    int fd;
    fd = open("/sys/class/backlight/ns-backlight/brightness", O_RDWR);
    if(fd >= 0) {
        char buffer[20];
	int bytes, amt;
        if(delayms) {
...
static int logoname_get(char *logoname)
{
    char lcdres[16] = {0};
    int fd;
    strcpy(logoname, INIT_IMAGE_FILE);
    fd = open("/sys/class/livall/state/lcdres", O_RDONLY);
    if(fd < 0) return -EEXIST;
    read(fd, lcdres, sizeof(lcdres));
    close(fd);
    sprintf(logoname, "initlogo_%s.rle", lcdres);
    fd = open(logoname, O_RDONLY);
    if (fd < 0) {
    	return -EEXIST;
    }
    close(fd);
...
static int console_init_action(int nargs, char **args)
{
    ...
    if(0 == logoname_get(tmp)) {
    	backlight_control(0, 0);
    }
    if( load_565rle_image(tmp) ) {
