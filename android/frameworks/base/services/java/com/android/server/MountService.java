...
class MountService extends IMountService.Stub
    ...
    private static final boolean LOCAL_LOGD = /*false*/true;
    private static final boolean DEBUG_UNMOUNT = /*false*/true;
    private static final boolean DEBUG_EVENTS = /*false*/true;
    private static final boolean DEBUG_OBB = /*false*/true;
                    ...
                    public void run() {
                        try {
                            ...
                            Slog.e(TAG, "Boot-time mount exception", ex);
                                                                                                                                                                             ...
                                                                                                                                                                             int adbOn = 0;
                try{
                       adbOn = Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.ADB_ENABLED);
                   } catch (android.provider.Settings.SettingNotFoundException e) {
                       e.printStackTrace();
                   }
                                                                                                                                                                             boolean available = (intent.getBooleanExtra(UsbManager.USB_CONNECTED, false) &&
                                     intent.getBooleanExtra(UsbManager.USB_FUNCTION_MASS_STORAGE, false) && (adbOn<1));
    ...
    public void onDaemonConnected() {
        ...
        new Thread() {
            ...
            public void run() {
                 ...
                 */
                String ExceptionPath = "/mnt/sdcard";
                try {
                    ...
                    for (String volstr : vols) {
                        ...
                        Slog.i(TAG, String.format("<label> <mountpoint> <state> <%s> <%s> <%s> ", tok[0], tok[1], tok[2]));
                        String path = tok[1];
                        ExceptionPath = path;
                    ...
                    //luwenchao fix bug 0003161
                    updatePublicVolumeState(ExceptionPath, Environment.MEDIA_REMOVED);
                }
...
// @TEI modified to fix bug #0002055
// when rush to remove sd card, Upper logic layer should umount it.
                if (DEBUG_EVENTS) Slog.i(TAG, "unmountVolume");
		unmountVolume(path, true, false);
// @TEI modified end
                    return true;
		}
    ...
    private int doMountVolume(String path) {
        ...
        try {
            ...
            if (code == VoldResponseCode.OpFailedNoMedia) {
                 ...
                 */
//@LWC delete,aviod checking err because of we don't have check fs
/*
...
*/
    ...
    private void notifyShareAvailabilityChange(final boolean avail) {
        ...
        final String TFcard_path = Environment.getTFcardStorageDirectory().getPath();

        if (avail == false && (getVolumeState(path).equals(Environment.MEDIA_SHARED)||getVolumeState(TFcard_path).equals(Environment.MEDIA_SHARED))) {
            /*
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    if(getVolumeState(TFcard_path).equals(Environment.MEDIA_SHARED)){
                        try {
                             int rc;
                             Slog.w(TAG, "Disabling UMS after cable disconnect");
                             doShareUnshareVolume(TFcard_path, "ums", false);
                             if ((rc = doMountVolume(TFcard_path)) != StorageResultCode.OperationSucceeded) {
                                 Slog.e(TAG, String.format(
                                     "Failed to remount {%s} on UMS enabled-disconnect (%d)",
                                            TFcard_path, rc));
                                 }
