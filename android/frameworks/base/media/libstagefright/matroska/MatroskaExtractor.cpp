...
private:
    ...
    enum Mode {
        ...
        COMPRESSION_BZLIB          = 1,
        COMPRESSION_LZO1X          = 2,
        COMPRESSION_HEAD_STRIPPING = 3
    };
    struct Compression {
        int32_t algo;
        size_t settings_size;
        const void* settings_data;
    };
    /*a@nufront end*/
    ...
    size_t mTrackIndex;
    Type mType;
    Compression mCompression; /*a@nufront*/
    bool mIsAudio;
...
MatroskaSource::MatroskaSource(
    ...
    /*a@nufront start*/
    uint32_t type;
    memset(&mCompression, COMPRESSION_NONE, sizeof(Compression));
    meta->findInt32(kKeyCompAlgo, &(mCompression.algo));
    meta->findData(kKeyCompSettings, &type,
            &(mCompression.settings_data), &(mCompression.settings_size));
    /*a@nufront end*/
...
status_t MatroskaSource::readBlock() {
    ...
    for (int i = 0; i < block->GetFrameCount(); ++i) {
        ...
        /*a@nufront start*/
        timeUs = (i == 0 ? timeUs : INVALID_TIMESTAMP);
        /*a@nufront end*/
...
again:
    ...
    /*m@nufront start:
     * Add Decompression Process.
     * We can only deal with header stripping compression now.*/
    MediaBuffer *bufferSrc = NULL;
    switch (mCompression.algo) {
        case COMPRESSION_NONE:            // no compression
        case COMPRESSION_ZLIB:            // zlib is not supported
        case COMPRESSION_BZLIB:           // bzlib is not supported
        case COMPRESSION_LZO1X:           // lzo1x is not supported
            break;
        case COMPRESSION_HEAD_STRIPPING:  // header stripping is supported.
            if (mCompression.settings_data == NULL
                    || mCompression.settings_data <= 0) {
                // no settings data found
                break;
            }
            bufferSrc = new MediaBuffer(srcSize + mCompression.settings_size);
            memcpy(bufferSrc->data(),
                    (void*)mCompression.settings_data,
                    mCompression.settings_size);
            memcpy(bufferSrc->data() + mCompression.settings_size,
                    (void*)srcPtr, srcSize);
            srcSize = srcSize + mCompression.settings_size;
            srcPtr = (uint8_t *)bufferSrc->data();
            break;
        default:
            break;
    }
    ...
    if (mType != AVC) {
        if (srcSize > frame->range_length()) {
            int64_t timeUs;
            CHECK(frame->meta_data()->findInt64(kKeyTime, &timeUs));
            int32_t isSync;
            CHECK(frame->meta_data()->findInt32(kKeyIsSyncFrame, &isSync));

            bufferSrc->meta_data()->setInt64(kKeyTime, timeUs);
            bufferSrc->meta_data()->setInt32(kKeyIsSyncFrame, isSync);
    ...
    /*d@nufront start: move to line 466*//*
    const uint8_t *srcPtr =
        (const uint8_t *)frame->data() + frame->range_offset();
...
void MatroskaExtractor::addTracks() {
    ...
    for (size_t index = 0; index < tracks->GetTracksCount(); ++index) {
        ...
        /*a@nufront start*/
        size_t m_comp_settings_size;
        const unsigned char *m_comp_settings_data =
            track->GetCompSettings(m_comp_settings_size);
        meta->setInt32(kKeyCompAlgo, track->GetCompAlgo());
        meta->setData(kKeyCompSettings, 0,
            m_comp_settings_data, m_comp_settings_size);
        /*a@nufront end*/
        ...
        switch (track->GetType()) {
            case VIDEO_TRACK:
            {
                ...
                /*a@nufront start*/
                    ...
                    meta->setData(kKeyMPEG4InMKV, 0, codecPrivate, codecPrivateSize);
                /*a@nufront end*/
                } else {
                    continue;
                }
