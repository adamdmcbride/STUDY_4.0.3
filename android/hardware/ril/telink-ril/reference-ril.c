...
#define TELINK_7619_RIL_VERSION "TELINK-7619-RIL-W1.0.6"
...
static void requestRadioPower(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (onOff == 0 && sState != RADIO_STATE_OFF) {
        err = at_send_command("AT+CFUN=4", &p_response); //telink cfun=4 is airplane mode
...
static void requestGetCurrentCalls(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (countValidCalls) {  // We don't seem to get a "NO CARRIER" message from smd, so we're forced to poll until the call ends.
#else
    if (needRepoll) {
#endif
...
int getSignalStrength(RIL_SignalStrength_v6 *signalStrength)
{
    ...
    signalStrength->LTE_SignalStrength.signalStrength = 0xFFFFFFFF; //0x7FFFFFFF;
    signalStrength->LTE_SignalStrength.rsrp = 0xFFFFFFFF; //0x7FFFFFFF;
    signalStrength->LTE_SignalStrength.rsrq = 0xFFFFFFFF; //0x7FFFFFFF;
    signalStrength->LTE_SignalStrength.rssnr = 0xFFFFFFFF; //0x7FFFFFFF;
    signalStrength->LTE_SignalStrength.cqi = 0xFFFFFFFF; //0x7FFFFFFF;
...
static void requestSignalStrength(void *data, size_t datalen, RIL_Token t)
{
	...
	if (getSignalStrength(&signalStrength) < 0) {
		...
		LOGD("requestSignalStrength is %d and sizeof is %d",signalStrength.GW_SignalStrength.signalStrength, sizeof(RIL_SignalStrength_v6));
...
if(request == RIL_REQUEST_DATA_REGISTRATION_STATE && commas == 3) commas++;
if(request == RIL_REQUEST_DATA_REGISTRATION_STATE && commas == 3) commas++;
    switch (commas) {
        case 0: /* +CREG: <stat> */
    ...
    if (count > 3){
		err = at_send_command_singleline("AT+XREG?", "+XREG:", &p_response);
	    if (err != 0) goto error;
		...
		//4->9 ,5->10 convert for type defined in ril.h(RIL_RadioTechnology)
		if(response[3] == 4) response[3]=9;
		if(response[3] == 5) response[3]=10;
...
void requestScreenState(void *data, size_t datalen, RIL_Token t)
{
    ...
    /* To do :  need to set para to modem */
    /*   
       ...
       */
    if (ScreenOn) {
		//set all unsolicited report is enable
        err = at_send_command("AT+XCIND=4095", NULL);
    } else {
    	//just report service+message+call+simfull
		err = at_send_command("AT+XCIND=180", NULL);
    }    
    //free(cmd);
    RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
    at_response_free(p_response);
...
//Telink call request
		...
		case  RIL_REQUEST_GET_CLIR:
			requestQueryClirStatus(data, datalen, t);
			break;

		case  RIL_REQUEST_QUERY_CALL_WAITING:
			requestQueryCallWaitingStatus(data, datalen, t);
			break;
		...
		case RIL_REQUEST_SET_CALL_FORWARD:
			requestSetCallForwardStatus(data, datalen, t);
			break;


//telink network request
	    ...
	    case RIL_REQUEST_GET_IMEISV:
            p_response = NULL;
            err = at_send_command_numeric("AT+XCGSN", &p_response);
...
//Telink SIM request
        ...
        case RIL_REQUEST_SET_NETWORK_SELECTION_AUTOMATIC:
			//telink add for unsol signal strength after reg network
		...
		case RIL_REQUEST_SET_NETWORK_SELECTION_MANUAL:
			...
			//RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, NULL);
			break;
...
/* TeLink SIM Requests */
		...
		case RIL_REQUEST_SET_FACILITY_LOCK:
			requestSetFacilityLock(data, datalen, t,request);
			break;
...
{
    return TELINK_7619_RIL_VERSION;
}
...
static int getCardStatus(RIL_CardStatus_v6 **pp_card_status) {
    static RIL_AppStatus app_status_array[] = {
		...
		//telink just add pin2 and puk2
		//	SIM_PIN2 = 6,
        { RIL_APPTYPE_USIM, RIL_APPSTATE_READY, RIL_PERSOSUBSTATE_UNKNOWN,
...
static void initializeCallback(void *param)
{
    ...
    //telink
    err = at_send_command("AT+XRAT=2,2",NULL);
    if (err != 0)
        return ;
	err = at_send_command("AT+XREG=1", NULL);
    if (err != 0)
        return ;
    err = at_send_command("AT+XPBK=1",NULL);
    if (err != 0)
        return ;
	err = at_send_command("AT+XSMS=1",NULL);
    if (err != 0)
        return ;
	err = at_send_command("AT+XSIMSTATE=1", NULL);
    if (err != 0)
        return ;
    err = at_send_command("AT+XCIND=4095",NULL);
    if (err != 0)
    ...
    /*  HEX character set */
    at_send_command("AT+CSCS=\"UTF-8\"", NULL);
    ...
    /*  Open Modem log */
    at_send_command("AT+XSIO=1", NULL);
#endif
...
//telink add
	 else if (strStartsWith(s, "+XCALLSTAT:")) {
		RIL_onUnsolicitedResponse (RIL_UNSOL_DATA_CALL_LIST_CHANGED,NULL, NULL);
    } else if (strStartsWith(s, "+XSIM:")) {
    			//telink for XSIM Unsolicited
				int sim_status = 0;
				int response[2];
				line = strdup(s);
				at_tok_start(&line);
				...
				//LOGE("get XSIM line %d\n", sim_status);

				if (err < 0) {
					LOGE("invalid XSIM line %s\n", s);
				}
				switch(sim_status){
					case 4://puk
					case 1://pin
				...
				response[1] = 0;
				//LOGE("get XSIM response %d %d\n", response[0],response[1]);
				RIL_onUnsolicitedResponse (RIL_UNSOL_SIM_REFRESH,response, sizeof(response));
				//telink for XSIM Unsolicited end
    	RIL_onUnsolicitedResponse (RIL_UNSOL_RESPONSE_RADIO_STATE_CHANGED,NULL, NULL);
	} else if (strStartsWith(s, "+XSMS:")) {
		//RIL_onUnsolicitedResponse (RIL_UNSOL_SIM_REFRESH,NULL, NULL);
	} else if (strStartsWith(s, "+XPBK:")) {
		//RIL_onUnsolicitedResponse (RIL_UNSOL_SIM_REFRESH,NULL, NULL);
	} else if (strStartsWith(s, "+CIEV:")) {
		getSignalStrength(&signalStrength);
	    RIL_onUnsolicitedResponse(RIL_UNSOL_SIGNAL_STRENGTH,&signalStrength, sizeof(RIL_SignalStrength_v6));
	}	else if (strStartsWith(s, "+STKPRO:")) {
		reportStkProCommand(s);
	}else if (strStartsWith(s, "+STKEVT:")) {
		reportStkEvent(s);
	}
...
mainLoop(void *param)
{
	...
	LOGD("mainLoop s_device_path : %s",s_device_path);
    ...
    for (;;) {
        ...
        while  (fd < 0) {
            if (s_port > 0) {
				...
				LOGD("telink poll fd is %d \n",fd);
                if ( fd >= 0 && (memcmp(s_device_path, "/dev/ttyACM", 11) == 0)) {
