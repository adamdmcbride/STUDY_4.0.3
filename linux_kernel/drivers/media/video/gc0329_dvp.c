...
static int set_flip(struct i2c_client * client, int is_on)
{
	...
	//wtx 20120705
	//here the val will equal to 0x14
	//it will change the mirror value
	//so here ignore the val,write the right value 0x15
	ret = gc0329_write(client, 0x17, 0x10);
	if(ret < 0)
		return ret;
...
static int set_mirror(struct i2c_client * client, int is_on)
{
	...
	//wtx 20120705                                                                                               
	//here ignore the val,write the right value 0x15
	ret = gc0329_write(client, 0x17, 0x10);
	if(ret < 0)
		return ret;
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	msleep(100);//wait for camera stable
	return ret;
}
			      ...
			      struct i2c_client *client)
{
	 ...
	 */
	/*
	*do not check the camera ID,when the machine power on the I2C is not stable,
	*sometimes it will fail to read the camera ID,so allways fail to probe the camera IC.
	*/
	return 1;
#if 0
