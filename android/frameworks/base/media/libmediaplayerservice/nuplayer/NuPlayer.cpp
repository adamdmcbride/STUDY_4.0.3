    ...
    switch (msg->what()) {
        ...
        case kWhatAudioNotify:
        {
            ...
            if (what == ACodec::kWhatFillThisBuffer) {
                ...
                LCHLOGD2("%s shutdown completed", audio ? "audio" : "video");
                if (audio) {
                    mAudioDecoder.clear();
...
status_t NuPlayer::instantiateDecoder(bool audio, sp<Decoder> *decoder) {
    ...
    *decoder = audio ? new Decoder(notify) :
    ...
    LCHLOGD1("%s duration=%lld (%f s)", (audio ? "audio" : "video"), durationUs, (durationUs/1000000.0));
    ...
    do {
        ...
        if (err == -EWOULDBLOCK) {
            ...
            if (err == INFO_DISCONTINUITY) {
                ...
                LCHLOGD1("%s discontinuity (formatChange=%d, time=%d)", audio ? "audio" : "video", formatChange, timeChange);
        ...
        if (!audio
            ...
            LCHLOGD1("dropAccessUnit, mNumFramesDropped=%d", mNumFramesDropped);
            dropAccessUnit = true;
            ++mNumFramesDropped;
        }
