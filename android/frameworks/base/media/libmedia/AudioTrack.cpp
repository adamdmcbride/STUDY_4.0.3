...
const static int MAX_LOOP_COUNT = 10;
...
void AudioTrack::start()
{
    ...
    if (mActive == 0) {
        ...
        if (t != 0) {
           /*a@nufront start: if thread is running, retry serveral times */
           int ret = 0, loopcount = 0;
           do {
               ret = t->run("AudioTrackThread", ANDROID_PRIORITY_AUDIO);
               cblk->lock.unlock();
               mPauseCond.waitRelative(mLock, 200);
               cblk->lock.lock();
               loopcount++;
           } while (INVALID_OPERATION == ret && loopcount < MAX_LOOP_COUNT && mPauseFlag);
           /*a@nufront end */
        } else {
        ...
        bool enforceFrameCount)
{
    ...
    if (!audio_is_linear_pcm(format)) {
        ...
        // Ensure that buffer depth covers at least audio hardware latency
        uint32_t minBufCount = afLatency / ((1000 * afFrameCount)/afSampleRate);
        /*m@nufront start: modify to adjust buffer size for frame lost of playback*/
        if (minBufCount < 2) minBufCount = 2;
        if (minBufCount > 4) minBufCount = 4;
        /*m@nufront end*/
        ...
        if (sharedBuffer == 0) {
            ...
            if (frameCount < minFrameCount) {
                //TEL@---changed by zhangjun for issue 1497, refer to DEFAULT_AUDIOSINK_BUFFERCOUNT in MediaPlayInterface.h
                //enforceFrameCount = false;
    ...
    do {
        ...
        if (err < NO_ERROR) {
            if (err != TIMED_OUT) {
                ...
                /*m@nufront start: if AudioTrack::start is executed, mActive would be 1,
                 *in this case, not return. */
                AutoMutex lock(mLock);
                if (0 == mActive) {
                    mPauseCond.signal();
                    return false;
        ...
        /*m@nufront start: AudioTrack::start is waiting mPauseCond, need to signal */
        if (mPauseFlag) {
            AutoMutex lock(mLock);
            mPauseFlag = false;
            mPauseCond.signal();
