...
#define VDECDEVICE   "/dev/hx170"
/* Use 'k' as magic number */
#define HX170DEC_IOC_MAGIC  'k'
#define HX170DEC_IOC_GETMEMHEAPINFO  _IOWR(HX170DEC_IOC_MAGIC, 8, unsigned long)
...
int g_vdev = -1;
uint32_t g_vphysaddr = 0;
ump_handle g_vhandle = UMP_INVALID_MEMORY_HANDLE;
void *g_vbase = NULL;
uint32_t g_vump_id  = 0;

#else
/* To fetch framebuffer device handler&info, define global variables */
int g_fbdev = -1;
void *g_fbbase = NULL;
uint32_t g_physaddr = 0;
#endif
...
#ifdef BOARD_USES_HDMI
	int index = (hnd->base > m->framebuffer->base) ? 1 : 0;
#ifdef USE_GC_CONVERTER
	if (gcconverter_blit_ui(m, index) != 0)
		return 0;
#endif
	if (hdmi_post(m, index) != 0)
		return 0;
#endif
...
int init_frame_buffer_locked(struct private_module_t* module)
{
    ...
    // refreshRate is 60.57 computed from above, but we can not pass CTS because just achieved 58.33
    // sometimes only achieved 57.8
    if (fps > 2.0) fps -= 2.0;
...
#ifdef GRALLOC_VIDEO_MAP_UMP
	g_vdev = open(VDECDEVICE, O_RDWR, 0);
	if (g_vdev < 0) {
		LOGE("%s Error opening %s: %s", __FUNCTION__, VDECDEVICE, strerror(errno));
		goto ErrorReturn;
	}
	MemallocHeapInfo vinfo;
	memset(&vinfo, 0, sizeof(MemallocHeapInfo));
	if (ioctl(g_vdev, HX170DEC_IOC_GETMEMHEAPINFO, &vinfo) < 0) {
		LOGE("%s Error ioctl HX170DEC_IOC_GETMEMHEAPINFO on %s", __FUNCTION__, VDECDEVICE);
		goto ErrorReturn;
	}
	if (vinfo.secureID == UMP_INVALID_SECURE_ID) {
		LOGE("%s Error get invalid ump secure id: 0x%x", __FUNCTION__, vinfo.secureID);
		goto ErrorReturn;
	}
	g_vphysaddr = vinfo.physicalAddr;
	g_vump_id = vinfo.secureID;
	g_vhandle = ump_handle_create_from_secure_id(vinfo.secureID);
//	LOGD("ioctl: hx170dec memsize=0x%x, baseaddr=0x%x, secureid=0x%x, ump_handle_created=0x%x", vinfo.totalSize, vinfo.physicalAddr, vinfo.secureID);
	if ( UMP_INVALID_MEMORY_HANDLE == g_vhandle ) {
		LOGE("%s can't create ump handle from secure id: 0x%x", __FUNCTION__, vinfo.secureID);
		goto ErrorReturn;
	}
	g_vbase = (void *)ump_mapped_pointer_get(g_vhandle);
	if (NULL == g_vbase) {
		LOGE("%s can't get mapping ump pointer from handle=0x%x", __FUNCTION__, g_vhandle);
		goto ErrorReturn;
	}
#else
	g_fbdev = fd;
	g_fbbase = vaddr;
	g_physaddr = finfo.smem_start;
#endif

#ifdef USE_GC_CONVERTER
	if (gcconverter_init(module) != 0)
		return 0;
#endif

#ifdef BOARD_USES_HDMI
	if (hdmi_init(module)!= 0)
		return -1;
#endif
...
ErrorReturn:
	if (g_vbase != NULL) {
		ump_mapped_pointer_release(g_vhandle);
		g_vbase = NULL;
	}
	if ( UMP_INVALID_MEMORY_HANDLE != g_vhandle ) {
		ump_reference_release(g_vhandle);
		g_vhandle = UMP_INVALID_MEMORY_HANDLE;
	}
	if (g_vdev > 0) {
		close(g_vdev);
		g_vdev = -1;
