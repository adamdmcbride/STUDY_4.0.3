...
void MPEG2TSExtractor::init() {
    ...
    /*a@nufront start: for bug 1175, record origin offset*/
    off64_t tmpAudioOffset = 0;
    off64_t tmpVideoOffset = 0;
    /*a@nufront end*/
    while (feedMore() == OK) {
        ...
        if (!haveAudio) {
            ...
            if (impl != NULL) {
                ...
                mSourceImpls.push(impl);
                /*a@nufront start: for bug 1175, record origin offset*/
                tmpAudioOffset = mOffset;
                /*a@nufront end*/
            }
        }

        /*m@nufront start: modify for programStatus get*/
        /*if (++numPacketsParsed > 10000) {
            break;
        }*/
        if (mParser->getProgramStatus()) {
            break;
        }
        /*m@nufront end*/
    }

    /*a@nufront start: bug 1175: resume wile file is error to handle error*/
    if (!(haveAudio && haveVideo)
        && !(mParser->getProgramStatus())) {
        if (haveAudio && !haveVideo) {
            mOffset = tmpAudioOffset;
        }
        if (!haveAudio && haveVideo) {
            mOffset = tmpVideoOffset;
        }
    }
    /*a@nufront end*/
