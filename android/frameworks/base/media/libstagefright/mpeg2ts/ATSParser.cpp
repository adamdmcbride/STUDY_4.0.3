...
/*a@nufront start: add header file for nufront log*/
...
struct ATSParser::Program : public RefBase {
    ...
    /*a@nufront start: add for record stream status*/
    bool getStreamsStatus() {
        return mStreamsReady;
    }
    /*a@nufront end*/
...
private:
    ...
    uint64_t mFirstPTS;
    /*a@nufront start: add for record stream status*/
    KeyedVector<unsigned, bool> mStreamStatus;
    bool mStreamsReady;
    /*a@nufront end*/
...
struct ATSParser::Stream : public RefBase {
    ...
    sp<MediaSource> getSource(SourceType type);
    /*a@nufront start: add to indict get stream data*/
    bool isFormatSupport() {
        return mFormatSupport;
    }
    /*a@nufront end*/
...
ATSParser::Program::Program(
    ...
    : mParser(parser),
      ...
      /*a@nufront start: bug1175, add to record stream status*/
      mStreamsReady(false),
      /*a@nufront end*/
      mFirstPTS(0) {
...
sp<MediaSource> ATSParser::Program::getSource(SourceType type) {
    ...
    for (size_t i = 0; i < mStreams.size(); ++i) {
        ...
        if (source != NULL) {
            /*a@nufront start: add for determine whice program to be play*/
            ssize_t streamIndex = mStreamStatus.indexOfKey(i);
            if (streamIndex < 0) {
                mStreamStatus.add(i, true);
                if (mStreamStatus.size() == mStreams.size()) {
                    mStreamsReady = true;
                }
            }
            /*a@nufront end*/
        ...
        /*a@nufront start: handle the satuation of unsupported format*/
        else if (!(mStreams.editValueAt(i)->isFormatSupport())) {
            mStreamStatus.add(i, true);
            if (mStreamStatus.size() == mStreams.size()) {
                mStreamsReady = true;
            }
...
ATSParser::Stream::Stream(
    ...
    : mProgram(program),
      ...
      /*a@nufront start: add to indicate whether get stream data*/
      mFormatSupport(true),
      mPrerollUs(0),
      /*a@nufront end*/
      mQueue(NULL) {
    switch (mStreamType) {
        ...
        default:
            /*a@nufront start: add to indicate whether get stream data*/
            ZJFLOGE("unsupport format type: 0x%x", mStreamType);
            LCHLOGE("unsupport format type: 0x%x", mStreamType);
            mFormatSupport = false;
            /*a@nufront end*/
            break;
    }
...
void ATSParser::Stream::signalDiscontinuity(
    ...
    /* a@nufront start */
    {
        ...
        if((extra != NULL) && (extra->findInt64("jump-at-prerollUs", (int64_t *) &mediatimeUs)))
        {
            mPrerollUs = mediatimeUs;
            LCHLOGD1("jump preroll=%f, return", (mediatimeUs/1000000.0));
            return;
        }
    }
    /* a@nufront end */
    ...
    /* a@nufront start */
    int64_t mediatimeUs = 0;
    if((extra != NULL) && (extra->findInt64("resume-at-prerollUs", (int64_t *) &mediatimeUs)))
    {
        mPrerollUs = mediatimeUs;
        LCHLOGD1("preroll=%f", (mediatimeUs/1000000.0));
    }
    /* a@nufront end */
...
void ATSParser::Stream::onPayloadData(
    ...
    /* LCHLOGV("onPayloadData mStreamType=0x%02x", mStreamType); */
...
ATSParser::ATSParser(uint32_t flags)
    /*m@nufront start: add for determine whice program to be play*/
    /*mFlags(flags){*/
    : mFlags(flags),
      mProgramReady(false),
      mWhich(-1) {
...
void ATSParser::signalDiscontinuity(
        DiscontinuityType type, const sp<AMessage> &extra) {
    LCHLOGD1("type = %d, what = %d", (int ) type, ((extra == NULL) ? -1 : (int ) extra->what()));
...
void ATSParser::signalEOS(status_t finalResult) {
    ...
    LCHLOGD1("finalResult = %d", (int ) finalResult);
...
sp<MediaSource> ATSParser::getSource(SourceType type) {
    ...
    for (size_t i = 0; i < mPrograms.size(); ++i) {
        ...
        if (mWhich >= 0 && (int)program->number() != mWhich) {
        /*m@nufront end*/
            continue;
        }
        ...
        if (source != NULL) {
            /*a@nufront start: add for determine whice program to be play*/
            if (mWhich == -1) {
                mWhich = (int)program->number();
            }

            if (program->getStreamsStatus()) {
                mProgramReady = true;
            }
            /*a@nufront end*/
