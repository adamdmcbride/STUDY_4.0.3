...
public:
	...
	// mapExternalMem attempts to map an external memory into framebuffer area.
	// This should be called before queueBuffer, and you should make sure that
	// the external memory could be mapped by eglCreateImageKHR. It's only used
	// to avoid memcpy from external memory to gralloc memory when video renders.
	//
	// This method will fail if gralloc_module_t:perform() hasn't been implemented.
	virtual status_t mapExternalMem(unsigned int physAddr, size_t size);

    // transformExternalMem attempts to transfer an external memory into framebuffer
    // area and convert data format into RGB format for bandwidth limit.
    // This should be called before queueBuffer, and you should make sure that
    // the external memory could be mapped by eglCreateImageKHR. It's only used
    // to avoid memcpy from external memory to gralloc memory when video renders.
    // And you should make sure there's a hardware color-converter to do the convert.
    //
    // This method will fail if gralloc_module_t:perform() hasn't been implemented.
...
private:
	...
	// mExtMemMapped is a flag that indicates if mapExternalMem() succeeded or failed.
	// mExtMemHandle holds the handle from mapExternalMem().
	bool mExtMemMapped;
	native_handle_t* mExtMemHandle;
	int mExtWidth;
	int mExtHeight;
	int mExtUsage;
