        ...
        sp<AMessage> *)
{
    ...
    uint8_t header[4+4];
   /*m@nufront start change the comparition condition of the first eight bytes */
   /*there may be many metadata_blocks,when the metadata_block whose type is stream_info is the last metadata_block ,header[4] is 0x80,otherwise header[4] is 0x00*/
   /* if (source->readAt(0, header, sizeof(header)) != sizeof(header)
            || memcmp("fLaC\0\0\0\042", header, 4+4))*/
      if (source->readAt(0, header, sizeof(header)) != sizeof(header)
             || (memcmp("fLaC\0\0\0\042", header, 4+4)&&memcmp("fLaC\200\0\0\042", header, 4+4)))
   /*m@nufront end change the comparition condition of the first eight bytes*/
    {
        return false;
    }
