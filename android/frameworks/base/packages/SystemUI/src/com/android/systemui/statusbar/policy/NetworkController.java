...
public class NetworkController extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(WifiManager.RSSI_CHANGED_ACTION)
            ...
            if(DEBUG)
            	Slog.d(TAG, "---onReceive()--SIM_STATE_CHANGED---XUE");
            updateDataIcon();
            refreshViews();
    ...
    private final void updateTelephonySignalStrength() {
        if (!hasService()) {
            ...
            //TEL@ For No 3G
            if (SystemProperties.get("connect.3g").equals("close")) {
              mPhoneSignalIconId = 0;// R.drawable.stat_sys_signal_0;
              mDataSignalIconId = 0;//R.drawable.stat_sys_signal_0;
            }
        } else {
    ...
    void refreshViews() {
        ...
        if (!mAirplaneMode) {
            if (SystemProperties.get("connect.3g").equals("close")) {
               mPhoneSignalIconId = mDataSignalIconId = 0;//R.drawable.stat_sys_signal_0; 
            }
        }
        ...
        if (mAirplaneMode &&
            ...
            if ((isCdma() && isCdmaEri()) || mPhone.isNetworkRoaming()) {
                mDataTypeIconId = R.drawable.stat_sys_data_connected_roam;
            }else if(mSimState == IccCard.State.ABSENT){
            	if(DEBUG)
            		Slog.d(TAG, "no service---no sim---XUE");
            	label = context.getString(R.string.accessibility_no_sim) + "|" + 
            			Resources.getSystem().getText(com.android.internal.R.string.emergency_calls_only)
            			.toString();
            	mDataTypeIconId = R.drawable.stat_sys_no_sim;
        ...
        //In order to solve the G icon and wifi icon overlay 
        if(mWifiConnected)
        {
        	N=mDataTypeIconViews.size();
        	for(int i=0;i<N;i++)
