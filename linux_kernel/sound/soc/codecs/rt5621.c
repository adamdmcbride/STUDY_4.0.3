...
static struct rt5621_reg init_data[] = {
 	{RT5621_GPIO_PIN_SHARING	,0xc001/*0x3*/},	//Output Mixer Control
 	//{RT5621_JACK_DET_CTRL		,0x8810},//jd1 detect
 	{RT5621_JACK_DET_CTRL		,0x8160},//jd1 detect
	{RT5621_GPIO_PIN_POLARITY	,0x4},
	{RT5621_GPIO_PIN_STICKY	,0x4},
	{RT5621_GPIO_PIN_WAKEUP	,0x4},	
	{RT5621_MONO_AUX_OUT_VOL	,0x0000},//output
	{RT5621_STEREO_DAC_VOL		,0x0101},//Stereo DAC volume
	{RT5621_ADC_REC_MIXER		,0x3f3f},//ADC Record Mixer Control
	//{RT5621_OUTPUT_MIXER_CTRL	,0x4700},//Output Mixer Control
	{RT5621_OUTPUT_MIXER_CTRL	,0x4740},//Output Mixer Control
	{RT5621_MIC_CTRL		,0x0500},//Microphone Control,boost +20db
	{RT5621_ADD_CTRL_REG		,0x5300},//General Purpose Control
	...
	struct snd_soc_dai *codec_dai)
{
	...
	dbg("rt5621_pcm_hw_params, iface=0x%x\n", iface);
	if (coeff < 0)
...
/*
        for(i=0; i<0x80;i+=2)
        {
                printk("rt5621[0x%x] = 0x%x\n", i, rt5621_read(codec,i));
        }
*/
	return 0;
}
...
//		return 0;
	...
	printk(KERN_EMERG "rt5621_suspend\n");
...
static int rt5621_resume(struct platform_device *pdev)
{
	...
	g_suspend_prepare = 0;
	printk(KERN_EMERG "rt5621_resume\n");
...
static int i2c_master_send_(struct i2c_client *client,const char *buf ,int count)
{
	if(g_suspend_prepare == 1) {
	//	printk("i2c_master_send fail\n");
		return -1;
	}
...
static int rt5621_probe(struct platform_device *pdev)
{
	...
	if (setup->i2c_address) {
		codec->hw_write = (hw_write_t)i2c_master_send_;
...
static int rt5621_sleep_pm_callback(struct notifier_block *nfb,
					unsigned long action,
					void *ignored);


static struct notifier_block rt5621_sleep_pm_notifier = {
	.notifier_call = rt5621_sleep_pm_callback,
	.priority = 0,
};
...
{
	g_suspend_prepare = 0;
	register_pm_notifier(&rt5621_sleep_pm_notifier);
...
static void __exit rt5621_exit(void)
{
	...
	unregister_pm_notifier(&rt5621_sleep_pm_notifier);
}
