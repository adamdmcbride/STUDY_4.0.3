...
void MediaPlayer::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    case MEDIA_PREPARED:
        LOGV("prepared");
        LCHLOGD1("prepared, mPrepareSync=%d", (int) mPrepareSync);
    ...
    case MEDIA_PLAYBACK_COMPLETE:
        ...
        LCHLOGD1("playback complete, mCurrentState=%d, mLoop=%d", (int) mCurrentState, (int) mLoop);
        if (mCurrentState == MEDIA_PLAYER_IDLE) {
    ...
    case MEDIA_ERROR:
        ...
        LCHLOGE("error (%d (0x%x), %d (0x%x))", ext1, ext1, ext2, ext2);
        mCurrentState = MEDIA_PLAYER_STATE_ERROR;
        if (mPrepareSync)
        {
            LOGV("signal application thread");
    ...
    case MEDIA_SEEK_COMPLETE:
        ...
        LCHLOGD1("Received seek complete, mSeekPosition=%d, mCurrentPosition=%d", mSeekPosition, mCurrentPosition);
    ...
    case MEDIA_BUFFERING_UPDATE:
        ...
        LCHLOGD1("buffering %d", ext1);
        break;
    case MEDIA_SET_VIDEO_SIZE:
        LOGV("New video size %d x %d", ext1, ext2);
        LCHLOGD1("New video size %d x %d, previos size %d x %d", ext1, ext2, mVideoWidth, mVideoHeight);
        mVideoWidth = ext1;
        mVideoHeight = ext2;
        break;
    case MEDIA_TIMED_TEXT:
        LOGV("Received timed text message");
        LCHLOGD1("Received timed text message");
        break;
    default:
        LOGV("unrecognized message: (%d, %d, %d)", msg, ext1, ext2);
        LCHLOGD1("unrecognized message: (%d, %d, %d)", msg, ext1, ext2);
        break;
    }
