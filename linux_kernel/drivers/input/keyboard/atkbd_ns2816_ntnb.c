			...
			unsigned int flags)
{
	...
	unsigned short keycode_ex, i;

	//printk("atkbd.c: Received %02x flags %02x\n", data, flags);
	keycode_ex = data;
#ifdef ATKBD_DEBUG
	...
	//printk("before atkbd_compat_scancode, code is 0x%x.\r\n", code);
	switch(code)
	{
		case 0x1C:
			if (keycode_ex != 0x9C) {
				//printk("key code 0x79, enter key.\r\n");
				keycode = 28;//Enter		
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
			}
			break;
		case 0x4B:
			if (keycode_ex != 0xCB) {
				//printk("key code 0x50, left key.\r\n");
				keycode = 105;//PAD left key	
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
			}
		...
		case 0x4D:
			if (keycode_ex != 0xCD) {
				//printk("key code 0x50, right key.\r\n");
				keycode = 106;//PAD right key	
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
			}
		...
		case 0x50:
			if (keycode_ex != 0xD0) {
				//printk("key code 0x50, down key.\r\n");
				keycode = 108;//PAD down key	
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
			}
		...
		case 0x48:
			if (keycode_ex != 0xC8) {
				//printk("key code 0x48, up key.\r\n");
				keycode = 103;//PAD up key		
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
			}
		...
		case 0x7B://vol down
			if (keycode_ex != 0xFB) {
				//printk("key code 0x7B, search key.\r\n");
				keycode = 217;//search
				input_event(dev, EV_KEY, keycode, 1);
				input_event(dev, EV_KEY, keycode, 0);
				input_sync(dev);
 ...
 */
	...
	if (!ps2_is_keyboard_id(param[0])) {
		printk("keyboard id match fail, keyboard_id = %x\n", param[0]);
		return -1;
}
