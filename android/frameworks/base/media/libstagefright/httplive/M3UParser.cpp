...
status_t M3UParser::parse(const void *_data, size_t size) {
    ...
    for(size_t i = 0x0; i < mItems.size(); i++) {
        AString uri;

        if(!itemAt(i, &uri, &itemMeta)) {
            LCHLOGD1("fail to get item %d", i);
            last_time = -1;
            continue;
        }
        ...
        if (itemMeta->findInt32("discontinuity", &val) && val != 0) {
            LCHLOGD1("get item discontinuity %d", i);
            val = 1;
        }

        const char* str = uri.c_str();
        const char* start = strstr(str, "start=");
        const char* end = strstr(str,"end=");
        int curr_time = -1;

        if(start && end) {
            int time_start = strtoul(start+6,NULL,10);
            int time_end = strtoul(end+4, NULL, 10);
            curr_time = (time_end > time_start) ? (time_start + time_end) : -1;
            LCHLOGD1("i=%d, time_start=%d, time_end=%d, val=%d, last_time=%d", i, time_start, time_end, val, last_time);
        }

        if((0x1 != val) && (-1 != last_time) && (-1 != curr_time) && (last_time > (curr_time + 100))) {
            itemMeta->setInt32("ptsdiscontinuity", true);
            Item *item = &mItems.editItemAt(i);
            item->mMeta = itemMeta;
            LCHLOGD1("set pts discontinuity, i=%d, curr_time=%d, last_time=%d", i, curr_time, last_time);
        }
...
status_t M3UParser::parseMetaDataDuration(
    ...
    (*meta)->setInt64(key, (int64_t) (x * 1000000.0));
    /* m@nufront end */
