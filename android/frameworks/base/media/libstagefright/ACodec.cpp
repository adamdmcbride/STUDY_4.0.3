...
status_t ACodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    LCHLOGD1("%s %s port allocate buffers", mComponentName.c_str(), (portIndex == kPortIndexInput ? "input" : "output"));
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i) {
        ...
        /*a@nufront start: Add process for our VPU.*/
#if VPU_SUPPORT
        } else if (!strcasecmp(
                    mComponentName.c_str(), "OMX.NU.Video.Decoder")) {
            err = mOMX->allocateBufferWithBackup(
                    mNode, portIndex, mem, &buffer);
#endif
        /*a@nufront end*/
...
status_t ACodec::allocateOutputBuffersFromNativeWindow() {
    YYLOGD("allocateOutputBuffersFromNativeWindow in!");
    ...
    if(!strcasecmp(mComponentName.c_str(),"OMX.NU.Video.Decoder")){
    ...
    err = native_window_set_buffers_geometry(
            ...
            halColorFormat);
    LCHLOGD1("%s : halColorFormat=%d", mComponentName.c_str(), halColorFormat);
#else
    /*a@nufront end*/
    err = native_window_set_buffers_geometry(
            mNativeWindow.get(),
    ...
    YYLOGD("before useGraphicBuffer:nBufferCountActual=%d",def.nBufferCountActual);
    for (OMX_U32 i = 0; i < def.nBufferCountActual; i++) {
...
status_t ACodec::freeOutputBuffersNotOwnedByComponent() {
    YYLOGD("freeOutputBuffersNotOwnedByComponent in!");
    for (size_t i = mBuffers[kPortIndexOutput].size(); i-- > 0;) {
        BufferInfo *info =
            &mBuffers[kPortIndexOutput].editItemAt(i);
...
void ACodec::configureCodec(
    ...
    if (!strncasecmp(mime, "video/", 6)) {
        ...
        if(!strcmp(mComponentName.c_str(), "OMX.NU.Audio.Decoder"))
        {
            CHECK_EQ(setupNuAudioDecoder(numChannels, sampleRate, mime), (status_t)OK);
        }
        else
        {
...
status_t ACodec::setVideoPortFormatType(
    ...
    LCHLOGD1("%s: compressionFormat = %d, colorFormat = %d",
        mComponentName.c_str(), (int) compressionFormat, (int) colorFormat);
    YYLOGD("setVideoPortFormatType in!");
    ...
    for (;;) {
        ...
        if (err != OK) {
            YYLOGD("getParameter failed!err is %d",err);
            return err;
        }
    ...
    YYLOGD("setVideoPortFormatType out!");
    return err;
}
...
status_t ACodec::setSupportedOutputFormat() {
    ...
    CHECK(format.eColorFormat == OMX_COLOR_FormatYUV420Planar
           ...
           || format.eColorFormat == OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar    /* a@nufront  */
    ...
    LCHLOGD1("%s: compressionFormat = %d, colorFormat = %d",
        mComponentName.c_str(), (int) format.eCompressionFormat, (int) format.eColorFormat);
...
status_t ACodec::setupVideoDecoder(
    ...
    YYLOGD("setupVideoDecoder in!");
    LCHLOGD1("mime = %s, size %d x %d", mime, width, height);
    OMX_VIDEO_CODINGTYPE compressionFormat = OMX_VIDEO_CodingUnused;
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
        compressionFormat = OMX_VIDEO_CodingAVC;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4, mime)) {
        compressionFormat = OMX_VIDEO_CodingMPEG4;
        setMp4vFormat(0, 0, MP4DEC_MPEG4);
    ...
    YYLOGD("mime is %s",mime);
    status_t err = setVideoPortFormatType(
            kPortIndexInput, compressionFormat, OMX_COLOR_FormatUnused);

    if (err != OK) {
        YYLOGD("set  format type err=%d!",err);
        return err;
    }
    ...
    if (err != OK) {
        YYLOGD("set output format format err=%d!",err);
        return err;
    }
    ...
    if (err != OK) {
    YYLOGD("set input port format err=%d!",err);
        return err;
    }
    ...
    if (err != OK) {
    YYLOGD("set output port format err=%d!",err);
        return err;
    }
    YYLOGD("setupVideoDecoder out!");
...
status_t ACodec::setVideoFormatOnPort(
    ...
    LCHLOGD1("compressionFormat = %d, size %d x %d", (int) compressionFormat, width, height);
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
    ...
    switch (def.eDomain) {
        case OMX_PortDomainVideo:
        {
            ...
            if (mOMX->getConfig(
                ...
                LCHLOGD1("rect [%d, %d] (%d x %d), frame size %d x %d", rect.nLeft, rect.nTop,
                    rect.nWidth, rect.nHeight, videoDef->nFrameWidth, videoDef->nFrameHeight);
            }
            ...
            if((mFrameWidth > 0) && (mFrameHeight > 0)) {
                rect.nLeft = 0;
                rect.nTop = 0;
                rect.nWidth = mFrameWidth;
                rect.nHeight = mFrameHeight;
            }
            /* a@nufront end */
        ...
        case OMX_PortDomainAudio:
        {
            ...
            LCHLOGD1("ch=%d, sr=%d", params.nChannels, params.nSamplingRate);
...
void ACodec::signalError(OMX_ERRORTYPE error) {
    LCHLOGD1("error = 0x%x", (int ) error);

    /* a@nufront start, workround for #3293, doesn't stop playback when decoder occurs error, just skip this frame */
    if(OMX_ErrorHardware == error) {
        LCHLOGD1("ignore error = 0x%x", (int ) error);
        return;
    }
    /* a@nufront end */
...
bool ACodec::BaseState::onOMXMessage(const sp<AMessage> &msg) {
    ...
    LCHLOGV("type=%d, msg = %d", type, (int) msg->what());
...
bool ACodec::BaseState::onOMXEmptyBufferDone(IOMX::buffer_id bufferID) {
    ...
    info->mStatus = BufferInfo::OWNED_BY_US;
    LCHLOGV("[%s] onOMXEmptyBufferDone %p", mCodec->mComponentName.c_str(), bufferID);
...
void ACodec::BaseState::postFillThisBuffer(BufferInfo *info) {
    if (mCodec->mPortEOS[kPortIndexInput]) {
        LCHLOGD1("%s - mPortEOS", mCodec->mComponentName.c_str());
        return;
    }
...
void ACodec::BaseState::onInputBufferFilled(const sp<AMessage> &msg) {
    ...
    LCHLOGV("bufferID = %d", (int) bufferID);
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    LCHLOGV("[%s] onOMXFillBufferDone %p time %lld us",mCodec->mComponentName.c_str(), bufferID, timeUs);
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            ...
            if (flags & OMX_BUFFERFLAG_EOS) {
                ...
                LCHLOGD1("%s - OMX_BUFFERFLAG_EOS, flags=0x%x", mCodec->mComponentName.c_str(), (unsigned int ) flags);
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    ...
    if (mCodec->mNativeWindow != NULL
        ...
        info->mStatus = BufferInfo::OWNED_BY_US;
        /*a@nufront start:
         * If we don't use native window to dequeue buffer for display,
         * we must add our renderer to render decoded data.*/
/*#if VPU_SUPPORT
        if (!strcasecmp("OMX.NU.Video.Decoder",
                    mCodec->mComponentName.c_str())
                    && mCodec->mRenderer != NULL) {*/
            /*The output picture is fragmentary if info->mData->size() == 1.*/
/*            if(info->mData->size() > 1){
                mCodec->mRenderer->render(info->mData->data(),
                        info->mData->size());
            }
        }
#endif*/
...
#if VPU_SUPPORT
struct ACodecHardwareRenderer : public ACodecRenderer {
    ACodecHardwareRenderer(
            const sp<ANativeWindow> &nativeWindow, const sp<MetaData> &meta)
        : mTarget(new HardwareRenderer(nativeWindow, meta)) {
    }

    virtual void render(const void *data, size_t size) {
    LCHLOGV("size = %d, data=%p", size, data);

        mTarget->ACodecRender(data, size, NULL);
    }

protected:
    virtual ~ACodecHardwareRenderer() {
        delete mTarget;
        mTarget = NULL;
    }
private:
    HardwareRenderer *mTarget;

    ACodecHardwareRenderer(const ACodecHardwareRenderer &);
    ACodecHardwareRenderer &operator=(const ACodecHardwareRenderer &);;
};
#endif
...
void ACodec::UninitializedState::onSetup(
    ...
    if(!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mime.c_str()))
    {
        ...
        if(msg->findInt32("channel-count", &numChannels) && (numChannels > 0x2))
        {
            FFmpegUtils::RegisterCodec((char* ) mime.c_str());
            //flags |= OMXCodec::kHardwareCodecsOnly;
            flags |= OMXCodec::kSoftwareCodecsOnly; //enable ffmpeg aac decoder after finishing aac decoder later
        }
        else
        {
    ...
    if (msg->findObject("native-window", &obj)
            /*m@nufront start: Replace google decoder to VPU.*/
            /*
#if VPU_SUPPORT
            && !strcasecmp("OMX.NU.Video.Decoder", componentName.c_str())) {
#else*/
...
/*#endif*/
        ...
        /*a@nufront start: Remove native window, but add our renderer for display.*/
        /*
#if VPU_SUPPORT
        mCodec->mNativeWindow = NULL;
        mCodec->mNativeWindow_VPU = nativeWindow->getNativeWindow();

        int32_t width, height;
        CHECK(msg->findInt32("width", &width));
        CHECK(msg->findInt32("height", &height));

        mCodec->mMetaData = new MetaData;
        mCodec->mMetaData->setInt32(kKeyWidth, width);
        mCodec->mMetaData->setInt32(kKeyHeight, height);
        mCodec->mMetaData->setInt32(kKeyColorFormat, mCodec->mColorFormat);
        mCodec->mMetaData->setRect(
            kKeyCropRect, 0, 0, width - 1, height - 1);

        mCodec->mRenderer = new ACodecHardwareRenderer(mCodec->mNativeWindow_VPU, mCodec->mMetaData);
#endif*/
        /*a@nufront end*/
    }
...
void ACodec::setMp4vFormat(int32_t width, int32_t height, MP4DecStrmFmt fmt)
{
...
void ACodec::setRVFormat(int32_t width, int32_t height, int32_t version) 
{
...
void ACodec::setVP6Format(int32_t flip)
{
...
void ACodec::setVC1Format(int32_t width, int32_t height, VC1DecStrmFmt fmt) 
{
...
status_t ACodec::setupNuAudioDecoder(int32_t numChannels, int32_t sampleRate, const char* mime)
{
    OMX_AUDIO_PARAM_NUAUDIOPROFILETYPE profile;
    memset(&profile, 0x0, sizeof(profile));

    status_t err = mOMX->getParameter(mNode, OMX_IndexParamAudioNuAudio, &profile, sizeof(profile));

    if (err != OK)
    {
        LCHLOGE("fail to get param, err=0x%x", err);
        return err;
    }

    if((!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mime)) && (!strcmp(mComponentName.c_str(), "OMX.NU.Audio.Decoder")))
    {
        FFmpegUtils::SetAudioProfile(&profile, mime);
    ...
    err = mOMX->setParameter(mNode, OMX_IndexParamAudioNuAudio, &profile, sizeof(profile));

    if (err != OK)
    {
        LCHLOGE("set NuAudioDec codecCtx error, err is %d", err);
    }
