...
struct OMXCodec : public MediaSource,
    ...
    enum CreationFlags {
        ...
        kOutputColorFormat565  = 512,
        /*a@nufront start: add a flag to specify preview image operation*/
        kPreviewImage          = 1024,
    };
...
/*a@nufront start*/
    typedef enum MP4DecStrmFmt_ {
        ...
        MP4DEC_SORENSON,
        MP4DEC_CUSTOM_1
    } MP4DecStrmFmt;  // hardware decoder needs more detail info about mpeg4

    typedef enum VC1DecStrmFmt_ {
        VC1DEC_VC1,
        VC1DEC_WMV3
    } VC1DecStrmFmt;  // hardware decoder needs more detail info about vc1
/*a@nufront end*/
...
/*a@nufront: start*/
#ifdef GOVERNOR_CTRL
    static uint32_t isDvfsFlag;
    static uint32_t initDvfsCnt;
    static char  dvfs_governor[DVFS_GOVERNOR_SIZE];
    static int32_t  readGovernor();
#endif
/*a@nufront: end*/
...
/*a@nufront start*/
    void setMpeg4Format(int32_t width, int32_t height, MP4DecStrmFmt fmt);
...
/*a@nufront end*/
    ...
    status_t setupVP8EncoderParameters(const sp<MetaData>& meta);/*a@nufront*/
    ...
    status_t setNuAudioDecoder(const sp<MetaData> &meta);
#ifdef WITH_APU_CODEC
    status_t setNuApuCodec(const sp<MetaData> &meta); /*add for apu component parameters set*/
#endif
    /*a@nufront end*/
