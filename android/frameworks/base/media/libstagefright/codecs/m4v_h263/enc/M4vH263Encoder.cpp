...
/*a@nufront: support camera inputbuffer struct as same as hard encoder*/
...
#if VPU_SUPPORT && defined (NS115)
    /*a@nufront: add param check,change data ptr to camera inputbuffer struct ptr*/
    if(mInputBuffer->size() != sizeof(camera_buf))
    {
        LOGE("inputbuffer size error");
        outputBuffer->release();
        mInputBuffer->release();
        mInputBuffer = NULL;
        return UNKNOWN_ERROR;
    }
    camera_buf *pIn = (camera_buf *)mInputBuffer->data();
    if (pIn->size - ((mVideoWidth * mVideoHeight * 3) >> 1) != 0) {
    /*a@nufront:end*/
#else
...
#if VPU_SUPPORT && defined (NS115)
    /*a@nufront: get real data address*/
    uint8_t *inPtr = (uint8_t *) pIn->vir_addr;
    /*a@nufront:end*/
#else
