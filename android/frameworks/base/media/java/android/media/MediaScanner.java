...
public class MediaScanner
{
    ...
    /* a@nufront: save scaner paths */
    private String[] mExternalStoragePaths;
    private String mVideoIndexPath = "/data/mediaserver/";
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
    ...
    private void pruneDeadThumbnailFiles() {
        /* a@nufront start: prune all storage paths's dead thumbnails file */
        for (int i = 0; i < mExternalStoragePaths.length; i++) {
            mExternalStoragePaths[i] += "/DCIM/.thumbnails";
        }
        HashSet<String> existingFiles = new HashSet<String>();
        for (int j = 0; j < mExternalStoragePaths.length; j++) {
            //String directory = "/sdcard/DCIM/.thumbnails";
            String directory = mExternalStoragePaths[j];
    ...
    public void scanDirectories(String[] directories, String volumeName) {
        try {
            /* a@nufront start: to save the scanner paths  */
            mExternalStoragePaths = directories;
            /* a@nufront end */
    ...
    public void deleteVideoIndex(String filePath) {
        String indexPathStart;
        String fullPath;
        int i = 0;
        try {
            String tempStr = filePath.replace('/', '_');
            if (tempStr.startsWith("_")) {
                indexPathStart = tempStr.substring(1);
            } else {
                indexPathStart = tempStr;
            }
            File files = new File(mVideoIndexPath);
            String[] filelist = files.list();
            if (filelist == null) {
                filelist = new String[0];
            }
            for (i = 0; i < filelist.length; i++) {
                if (filelist[i].startsWith(indexPathStart)) {
                    fullPath = mVideoIndexPath + filelist[i];
                    File indexFile = new File(mVideoIndexPath + filelist[i]);
                    indexFile.delete();
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "delete index path Exception:"+e.getMessage());
        }
    }

    /* a@nufront start: update media database */
    public void updateDatabase(String filePath, String volumeName)  {
        initialize(volumeName);
        Cursor c = null;
        String where = null;
        String[] selectionArgs = null;
        ...
        if (filePath != null) {
            where = MediaStore.MediaColumns.DATA+" like "+"'"+filePath+"/%'";
        } else{
            where = null;
        }
        try{
            // delect files where contain "filePath" from the mFilesUri table
            mMediaProvider.delete(mFilesUri, where, null);
        } catch (SQLException e) {
            // this might happen if the SD card is removed while the media scanner is running
            Log.e(TAG, "SQLException in MediaScanner.scan()", e);
        } catch (UnsupportedOperationException e) {
