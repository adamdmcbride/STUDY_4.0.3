...
public:
    ...
    /*a@nufront start: Convert bytes to jstring by the specified charset.*/
    jstring bytes2jstring(const char* bytes, const char* charset)
    {
        LOGV("bytes2jstring: bytes(%s) and charset(%s)", bytes, charset);
        jbyteArray bytesArray = NULL;
        jstring encoding = NULL;
        jstring dstString = NULL;
        jclass clazz = mEnv->FindClass("java/lang/String");
        jmethodID methodID = mEnv->GetMethodID(clazz, "<init>", "([BLjava/lang/String;)V");
        bytesArray = mEnv->NewByteArray(strlen(bytes));
        if (bytesArray == NULL) {
            goto clean_and_return;
        }
        mEnv->SetByteArrayRegion(bytesArray, 0, strlen(bytes), (jbyte*)bytes);
        encoding = mEnv->NewStringUTF(charset);
        if (encoding == NULL) {
            goto clean_and_return;
        }
        dstString = (jstring)mEnv->NewObject(clazz, methodID, bytesArray, encoding);

clean_and_return:
        if (bytesArray != NULL) {
            mEnv->DeleteLocalRef(bytesArray);
        }
        if (encoding != NULL) {
            mEnv->DeleteLocalRef(encoding);
        }
        if (dstString != NULL) {
            return dstString;
        }
    ...
    virtual status_t handleStringTag(const char* name, const char* value)
    {
        ...
        /*m@nufront start: We try to convert non-UTF8 value to gbk but not only display ??
         *                 for supporting chinese better.*/
        bool mark = false;
        ...
        if (mark) {
            valueStr = bytes2jstring(value, "gbk");
        } else {
