...
#endif
    mSource->stop();
    LOGD("AACEncoder after mSource->stop.");
...
/*a@nufront start*/
void AACEncoder::waitOutstandingEncodingFrames_l() {
    LOGV("waitOutstandingEncodingFrames_l: %lld", mNumClientOwnedBuffers);
    ZJFLOGD("function in, mNumClientOwnedBuffers is %lld, mStarted is %d.", mNumClientOwnedBuffers, mStarted);
    Mutex::Autolock autoLock(mLock);
    while (mNumClientOwnedBuffers > 0 || mStarted) {
        LOGD("before AACEncoder mFrameEncodingCompletionCondition.wait(mLock), mNumClientOwnedBuffers is %lld, mStarted is %d.", mNumClientOwnedBuffers, mStarted);
        mFrameEncodingCompletionCondition.wait(mLock);
        LOGD("after AACEncoder mFrameEncodingCompletionCondition.wait(mLock), mNumClientOwnedBuffers is %lld, mStarted is %d.", mNumClientOwnedBuffers, mStarted);
    }
}
void AACEncoder::signalBufferReturned(MediaBuffer *buffer) {
