...
final class BluetoothAdapterStateMachine extends StateMachine {
    ...
    private void broadcastState(int newState) {
        ...
        if (BluetoothAdapter.STATE_TURNING_ON == newState) {//add wakelock for bt S3
              mWakeLock.acquire();
              isWakeLockBT = true;
        }
        ...
        if ((BluetoothAdapter.STATE_TURNING_ON != mPublicState) && isWakeLockBT) {
            isWakeLockBT = false;
            mWakeLock.release();
        }
    }
