#ifndef NUSMART_EMAC_H
#define NUSMART_EMAC_H
#include <linux/phy.h>
#include <asm/io.h>
...
typedef struct DmaDescStruct
{
	...
	u32   buffer2;      /* Network Buffer 2 pointer or next descriptor pointer (Dma-able)in chain struct */
...
struct nusmartEMACDeviceStruct
{
	u32 queue_stopped;
	spinlock_t lock;
	u32 phyaddr;
...
struct nusmartEMACprivdataStruct{
	...
	struct net_device              		*nusmartEMACnetdev;
	struct nusmartEMACDeviceStruct		nusmartEMACdev;
	struct net_device_stats         	nusmartEMACNetStats;
	struct timer_list 			nusmartEMAC_cable_unplug_timer;
...
enum Mii_Phy_Status
{
	...
	Mii_phy_status_speed_100        = 0x2000,

	Mii_phy_status_full_duplex      = 0x0100,
...
enum DmaRegisters
{
	...
	PMTCASRegister	  = 0x002C,    /* CSR11 - PMT control and status register	    */
...
enum DmaControlReg
{
	...
	DmaRxStoreAndForward	= 0x02000000,
