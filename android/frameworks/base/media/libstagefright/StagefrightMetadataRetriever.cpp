...
static VideoFrame *extractVideoFrameWithCodecFlags(
        ...
        int64_t frameTimeUs,
        int seekMode) {
    /*m@nufront start: current operation is previewing image, set flag OMXCodec::kPreviewImage */
    sp<MediaSource> decoder =
        OMXCodec::Create(
                client->interface(), source->getFormat(), false, source,
                NULL, flags | OMXCodec::kClientNeedsFramebuffer | OMXCodec::kOutputColorFormat565 | OMXCodec::kPreviewImage);
    ...
    do {
        ...
        options.clearSeekTo();
             ...
             /*m@nufront start: buffer->range_length() may be 1 when decoding H264 stream*/
             || (buffer != NULL && buffer->range_length() <= 1));
             /*m@nufront end*/
    ...
    /*m@nufront: Width and height of decoder output picture may be not equal with
     *           width and height parsed from File extractor.
     *           We need do crop in this case.*/
    VideoFrame *frame = new VideoFrame;
    int32_t displayWidth, displayHeight;
    if (meta->findInt32(kKeyDisplayWidth, &displayWidth)) {
        frame->mDisplayWidth = displayWidth;
        if (crop_right > 0 && crop_right > displayWidth - 1) {
            crop_right = displayWidth - 1;
        }
    }
    if (meta->findInt32(kKeyDisplayHeight, &displayHeight)) {
        frame->mDisplayHeight = displayHeight;
        if (crop_bottom > 0 && crop_bottom > displayHeight - 1) {
            crop_bottom = displayHeight - 1;
        }
    }
