...
/*a@nufront start: add a renderer to call HardwareRenderer */
#if VPU_SUPPORT
struct AwesomeHardwareRenderer : public AwesomeRenderer {
    AwesomeHardwareRenderer(
...
protected:
    virtual ~AwesomeHardwareRenderer() {
        delete mTarget;
        mTarget = NULL;
    }
private:
    ...
    AwesomeHardwareRenderer(const AwesomeHardwareRenderer &);
    AwesomeHardwareRenderer &operator=(const AwesomeHardwareRenderer &);;
};
#endif
/*a@nufront end*/
...
/*a@nufront start*/
#ifdef IOCTL_MODIFY
    mFd=open("/dev/graphics/fb0",O_RDONLY,0);
#endif
#if VPU_SUPPORT
    USE_SURFACE_ALLOC = 0x0;
#else
    USE_SURFACE_ALLOC = 0x1;
#endif
/*a@nufront end*/
...
void AwesomePlayer::cancelPlayerEvents(bool keepNotifications) {
    LCHLOGV1("keepNotifications=%d", (int) keepNotifications);
    ...
    if (!keepNotifications) {
        LCHLOGV1("cancelEvent - mStreamDoneEvent, eventID=0x%x", mStreamDoneEvent->eventID());
...
void AwesomePlayer::reset() {
    /* a@nufront start */
    /* set SET_SEEK_EXIT_FLAG force ffmpeg seek exit */
    if(NO_SEEK != mSeeking && mVideoTrack != NULL) {
        mVideoTrack->set_seek_exit_flag(SET_SEEK_EXIT_FLAG);
    }
    /* a@nufront end */
    Mutex::Autolock autoLock(mLock);
    reset_l();
    /* a@nufront start */
    if(mVideoTrack != NULL) {
        mVideoTrack->set_seek_exit_flag(UNSET_SEEK_EXIT_FLAG);
    }
    /* a@nufront end */
}
...
void AwesomePlayer::onStreamDone() {
    ...
    if (mStreamDoneStatus != ERROR_END_OF_STREAM) {
        ...
        LCHLOGD1("MEDIA_ERROR %d", mStreamDoneStatus);
    ...
    LCHLOGD1("allDone=%d, mVideoSource=%d, mAudioSource=%d, mFlags=0x%x", (int) allDone, mVideoSource==NULL, mAudioSource==NULL, mFlags);
    if (!allDone) {
        return;
    }
    ...
    if ((mFlags & LOOPING)
        ...
        LCHLOGD1("MEDIA_PLAYBACK_COMPLETE");
...
void AwesomePlayer::notifyVideoSize_l() {
    ...
    /* a@nufront start , try to find the original frame size at first */
    if (meta->findRect(kKeyVideoRect, &cropLeft, &cropTop, &cropRight, &cropBottom)) {
        LCHLOGD1("got crop rect %d, %d, %d, %d",cropLeft, cropTop, cropRight, cropBottom);
    } else
    /* a@nufront end */
    if (!meta->findRect(
...
void AwesomePlayer::initRenderer_l() {
    LCHLOGD1("USE_SURFACE_ALLOC=%d, mNativeWindow=%d", USE_SURFACE_ALLOC,LCHISEXIST(mNativeWindow));
...
/*a@nufront start: if OMX.NU.Video.Decoder exist, use HardwareRenderer */
#if VPU_SUPPORT
    if ((!USE_SURFACE_ALLOC) && (!strncmp(component, "OMX.NU.Video.Decoder",
        sizeof("OMX.NU.Video.Decoder")))) {
        mVideoRenderer = new AwesomeHardwareRenderer(mNativeWindow, meta);
        return;
    }
#endif
/*a@nufront end*/
...
status_t AwesomePlayer::setSurfaceTexture(const sp<ISurfaceTexture> &surfaceTexture) {
    Mutex::Autolock autoLock(mLock);
    LCHLOGD1("surfaceTexture = %d", ((surfaceTexture != NULL) ? 1 : 0));
    status_t err;
...
status_t AwesomePlayer::setNativeWindow_l(const sp<ANativeWindow> &native) {
    LCHLOGD1("native = %d", ((native == NULL) ? 0 : 1));
...
void AwesomePlayer::onVideoEvent() {
    ...
    if (mSeeking != NO_SEEK) {
        ...
        if (SEEK == mSeeking && mAudioSource != NULL
        /*m@nufront end*/
                && !(mFlags & SEEK_PREVIEW)) {
...
/*a@nufront start*/
#ifdef IOCTL_MODIFY
            ioctl(mFd,FB_LOG_STAGEFRIGHT,0);
#endif
/*a@nufront end*/
    ...
    /*m@nufront start: mVideoBuffer->range_length() means decoder get a Picture
     * but might cause screen mess.
     */
    if (mVideoRenderer != NULL && mVideoBuffer->range_length() > 1) {
        mSinceLastDropped++;
...
void AwesomePlayer::postStreamDoneEvent_l(status_t status) {
    LCHLOGD1("mStreamDoneStatus=%d, mStreamDoneEventPending=%d",
          (int) mStreamDoneStatus, (int) mStreamDoneEventPending);
    if (mStreamDoneEventPending) {
        return;
    }
...
void AwesomePlayer::onCheckAudioStatus() {
    ...
    if (mWatchForAudioEOS && mAudioPlayer->reachedEOS(&finalStatus)) {
        LCHLOGD1("finalStatus=%d", (int) finalStatus);
        mWatchForAudioEOS = false;
...
status_t AwesomePlayer::finishSetDataSource_l() {
    ...
    if (!strncasecmp("http://", mUri.string(), 7)
        ...
        LCHLOGD2("contentType = %s", contentType.string());
            ...
            if (mCachedSource != NULL) {
                ...
                for (;;) {
                    ...
                    if (finalStatus != OK
                        ...
                        LCHLOGD2("finalStatus=%d, metaDataSize=%lld, cachedDataRemaining=%lld, mFlags=0x%x", (int ) finalStatus, metaDataSize, (off64_t)cachedDataRemaining, mFlags);
                        break;
                    }

                    LOGV("now cached %d bytes of data", cachedDataRemaining);
                    LCHLOGD2("now cached %d bytes of data", cachedDataRemaining);
                    ...
                    if (metaDataSize < 0
                        ...
                        LCHLOGD2("metaDataSize = %lld bytes", metaDataSize);
                    }
    ...
    LCHLOGD2("mUri = %s, sniffedMIME = %s", mUri.string(), (sniffedMIME.empty() ? "NULL" : sniffedMIME.c_str()));
