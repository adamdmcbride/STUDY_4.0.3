...
/*a@nufront start*/
void AMRNBEncoder::waitOutstandingEncodingFrames_l() {
    ZJFLOGD("function in.");
    LOGV("waitOutstandingEncodingFrames_l: %lld", mNumClientOwnedBuffers);
    Mutex::Autolock autoLock(mLock);
    while (mNumClientOwnedBuffers > 0 || mStarted) {
        ZJFLOGD("before mFrameEncodingCompletionCondition.wait, mNumClientOwnedBuffers is %lld, mStarted is %d.", mNumClientOwnedBuffers, mStarted);
        mFrameEncodingCompletionCondition.wait(mLock);
        ZJFLOGD("after mFrameEncodingCompletionCondition.wait, mNumClientOwnedBuffers is %lld, mStarted is %d.", mNumClientOwnedBuffers, mStarted);
    }
    ZJFLOGD("function out");
}
void AMRNBEncoder::signalBufferReturned(MediaBuffer *buffer) {
