...
/*m@nufront start: modify for playback frames lost*/
/* static */ int MediaPlayerService::AudioOutput::mMinBufferCount = 4;
/* static */ /*int MediaPlayerService::AudioOutput::mMinBufferCount = 1; */
        ...
        int audioSessionId, uid_t uid)
{
    ...
    LCHLOGD1("Client(%d) constructor", connId);
    mPid = pid;
    mConnId = connId;
    mService = service;
    mClient = client;
...
MediaPlayerService::Client::~Client()
{
    ...
    LCHLOGD1("Client(%d) destructor pid = %d", mConnId, mPid);
    mAudioOutput.clear();
        ...
        const char *url, const KeyedVector<String8, String8> *headers)
{
    ...
    if (strncmp(url, "content://", 10) == 0) {
        ...
        LCHLOGD1("mStatus=%d", (int) mStatus);
...
status_t MediaPlayerService::Client::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    LCHLOGD1("setDataSource fd=%d, offset=%lld, length=%lld", fd, offset, length);
    struct stat sb;
    int ret = fstat(fd, &sb);
    if (ret != 0) {
    ...
    LCHLOGD1("st_dev  = %llu, st_mode = %u,st_size = %llu", sb.st_dev,sb.st_mode,sb.st_size);
        ...
        const sp<ISurfaceTexture>& surfaceTexture)
{
    ...
    LCHLOGD1("[%d] setVideoSurfaceTexture(%p)", mConnId, surfaceTexture.get());
...
status_t MediaPlayerService::Client::isPlaying(bool* state)
{
    ...
    *state = p->isPlaying();
    ...
    //LCHLOGD1("[%d] isPlaying: %d", mConnId, *state);
    return NO_ERROR;
}
...
status_t MediaPlayerService::Client::getDuration(int *msec)
{
    ...
    LCHLOGV("getDuration %d (%f s)", *msec, (*msec/1000.0));
    return ret;
}
...
status_t MediaPlayerService::Client::seekTo(int msec)
{
    ...
    LCHLOGD1("[%d] seekTo %d (%f s)", mConnId, msec, (msec/1000.0));
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
status_t MediaPlayerService::Client::setAudioStreamType(int type)
{
    ...
    LCHLOGD1("[%d] setAudioStreamType(%d)", mConnId, type);
...
status_t MediaPlayerService::Client::setLooping(int loop)
{
    ...
    LCHLOGD1("[%d] setLooping(%d)", mConnId, loop);
    mLoop = loop;
    sp<MediaPlayerBase> p = getPlayer();
...
status_t MediaPlayerService::Client::setVolume(float leftVolume, float rightVolume)
{
    ...
    LCHLOGD1("[%d] setVolume(%f, %f)", mConnId, leftVolume, rightVolume);
    // TODO: for hardware output, call player instead
    Mutex::Autolock l(mLock);
...
status_t MediaPlayerService::Client::setAuxEffectSendLevel(float level)
{
    ...
    LCHLOGD1("[%d] setAuxEffectSendLevel(%f)", mConnId, level);
    Mutex::Autolock l(mLock);
...
status_t MediaPlayerService::Client::attachAuxEffect(int effectId)
{
    ...
    LCHLOGD1("[%d] attachAuxEffect(%d)", mConnId, effectId);
    Mutex::Autolock l(mLock);
...
status_t MediaPlayerService::Client::setParameter(int key, const Parcel &request) {
    ...
    LCHLOGD1("[%d] setParameter(%d)", mConnId, key);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
status_t MediaPlayerService::Client::getParameter(int key, Parcel *reply) {
    ...
    LCHLOGD1("[%d] getParameter(%d)", mConnId, key);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
        ...
        void* cookie, int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    LCHLOGD1("[%d] notify (%p, %d, %d, %d)", client->mConnId, cookie, msg, ext1, ext2);
