...
int SurfaceTextureClient::perform(int operation, va_list args)
{
    ...
    case NATIVE_WINDOW_API_DISCONNECT:
        ...
        break;
	case NATIVE_WINDOW_API_MAP_EXTERNALMEM:
		res = dispatchMapExternalMem(args);
		break;
	case NATIVE_WINDOW_API_TRANSFORM_EXTERNALMEM:
		res = dispatchTransformExternalMem(args);
		break;
    default:
        res = NAME_NOT_FOUND;
        break;
...
int SurfaceTextureClient::dispatchMapExternalMem(va_list args) {
	GraphicExternalMemInfo *param = va_arg(args, GraphicExternalMemInfo*);
	return mapExternalMem(param);
}

int SurfaceTextureClient::dispatchTransformExternalMem(va_list args) {
	TransformExternalMemInfo *param = va_arg(args, TransformExternalMemInfo*);
	return transformExternalMem(param);
}
...
//-------------------------------------------------------------------------
// mapExternalMem for mapping video hardware memory into frambuffer

int SurfaceTextureClient::mapExternalMem(GraphicExternalMemInfo *param)
{
	if (!param) {
		LOGE("%s NULL pointer of param", __FUNCTION__);
		return INVALID_OPERATION;
	}

	status_t err = mSurfaceTexture->mapExternalMem(param->physAddr, param->size);
	LOGE_IF(err, "%s Failed mapExternalMem res=%d", __FUNCTION__, err);
	return err;
}

//-------------------------------------------------------------------------
// transformExternalMem for converting and mapping video hardware memory into frambuffer
int SurfaceTextureClient::transformExternalMem(TransformExternalMemInfo *param)
{
	if (!param) {
		LOGE("%s NULL pointer of param", __FUNCTION__);
		return INVALID_OPERATION;
	...
	status_t err = mSurfaceTexture->transformExternalMem(
					param->srcPA, param->srcW, param->srcH, param->srcFmt, param->cropW, param->cropH,
					&param->outPA, &param->outW, &param->outH, &param->outFmt);
	LOGE_IF(err, "%s Failed transformExternalMem res=%d", __FUNCTION__, err);
	return err;
}
