...
// TEI@ add debug 
SLOGD("action=%d, devType=%s", action, devtype);    
            if (action == NetlinkEvent::NlActionAdd) {
                int major = atoi(evt->findParam("MAJOR"));
                int minor = atoi(evt->findParam("MINOR"));
                char nodepath[255];
				
/* TEI@ 2012.5.1 discard the workaroud related emmc boot0,1 bugs
char platform[10];
property_get("ro.board.platform", platform, NULL);

if (strcmp(platform, "ns2816") == 0) {
// @TEI workaround Bug #0001381: if (eMMC)mmcblk0boot0 or mmcblk0boot1, skip
    if (major == 179 && ( minor >= 16 && minor <= 32 )){
        // skip
        return 0;
    }
}
...
void DirectVolume::handleDiskAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    SLOGD("@handleDiskAdded MAJOR=%d, MINOR=%d", mDiskMajor, mDiskMinor);
    const char *tmp = evt->findParam("NPARTS");
    if (tmp) {
        mDiskNumParts = atoi(tmp);
        SLOGD("NPARTS=%d", mDiskNumParts);
    } else {
...
void DirectVolume::handlePartitionAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    if (part_num > mDiskNumParts) {
        SLOGD("part_num=%d, mDiskNumParts=%d", part_num, mDiskNumParts);
...
void DirectVolume::handleDiskChanged(const char *devpath, NetlinkEvent *evt) {
    ...
    if (tmp) {
        mDiskNumParts = atoi(tmp);
        SLOGD("disk change mDiskNumParts=%d", mDiskNumParts);
    } else {
        SLOGW("Kernel block uevent missing 'NPARTS'");
        mDiskNumParts = 1;
    }
...
//@TEI added begin to support automatic unmount OTI usb cable.
    int state;
...
int DirectVolume::getDeviceNodes(dev_t *devs, int max) {
    ...
    SLOGD("mPartIdx=%d,mDiskNumParts=%d,mDiskMajor=%d,mDiskMinor=%d", mPartIdx, mDiskNumParts, mDiskMajor, mDiskMinor);
    if (mPartIdx == -1) {
...
// @TEI BUG fix: when mDiskNumParts=0(no partition), but we misstake to assign number to partIndex
//  such as 1,2... instead of "auto" in vold.fstab, that may cause an unexpected result. 
    if (mPartIdx > mDiskNumParts ){
        devs[0] = MKDEV(mDiskMajor, mDiskMinor);
        return 1;
    }
// @TEI end
