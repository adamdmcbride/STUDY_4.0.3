...
public class IccLockSettings extends PreferenceActivity
    ...
    /*add by alex for show loft times of pin op*/
    private int LeftTimesPin = -1;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_ENABLE_ICC_PIN_COMPLETE:
                	/* add by alex for show left times of pin op*/
                    if((null != ar.exception) && (null != ar.result))
                    {
                    	LeftTimesPin = ((int[])ar.result)[0];
                    }
    ...
    private void iccLockChanged(boolean success) {
        if (success) {
        	...
        	/*modified by alex for toast left times
            Toast.makeText(this, mRes.getString(R.string.sim_lock_failed), Toast.LENGTH_SHORT)
                    .show();
                    */
        	Toast.makeText(this, mRes.getString(R.string.sim_lock_failed, LeftTimesPin), Toast.LENGTH_SHORT)
        	.show();
        }
        resetDialogState();
    }
    ...
    private void iccPinChanged(boolean success) {
        if (!success) {
        	/* modified by alex to show left times of pin op
            Toast.makeText(this, mRes.getString(R.string.sim_change_failed),
                    Toast.LENGTH_SHORT)
                    .show();
            */
        	Toast.makeText(this, mRes.getString(R.string.sim_change_failed, LeftTimesPin),
        	        Toast.LENGTH_SHORT)
        	        .show();
        } else {
