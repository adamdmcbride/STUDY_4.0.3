...
static const char *audio_interfaces[] = {
    /*m@nufront start: add for alsa*/
#ifdef WITH_GENERIC_AUDIO
    "primary",
#endif

#ifdef WITH_ALSA_AUDIO
    "alsa",
#endif
    "a2dp",
   // "usb",
...
status_t AudioFlinger::setParameters(int ioHandle, const String8& keyValuePairs)
{
    ...
    if (ioHandle == 0) {
        ZJFLOGD("setParameters for ioHandle == 0.");
        AutoMutex lock(mHardwareLock);
        mHardwareStatus = AUDIO_SET_PARAMETER;
        status_t final_result = NO_ERROR;
        for (size_t i = 0; i < mAudioHwDevs.size(); i++) {
            audio_hw_device_t *dev = mAudioHwDevs[i];
            ZJFLOGD("will call dev->set_parameters.");
            result = dev->set_parameters(dev, keyValuePairs.string());
            final_result = result ?: final_result;
            ZJFLOGD("after call dev->set_parameters, final_result is 0x%x.", final_result);
        }
        mHardwareStatus = AUDIO_HW_IDLE;
...
void AudioFlinger::PlaybackThread::readOutputParameters()
{
    ...
    /*a@nufront start: limit mFameCount to avoid frame lost*/
    if (mFrameCount > 2048) {
        mFrameCount  = 2048;
    }
    /*a@nufront end*/
