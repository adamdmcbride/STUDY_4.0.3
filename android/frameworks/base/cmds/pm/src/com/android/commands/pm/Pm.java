 ...
 */
/*
  2012-05-21 Add preapk support
  RULES: get params from filename(Filename must be start with "[c" or "[C")
    01. Params only support r/f/s/l/t
        r: reinstall apk, keep old data
        f: force installed on InternalStorage. It's not compatible with "s" param, use one of them first appear. If both of them are not, use "r' default
        s: force installed on ExternalStorage. It's not compatible with "s" param, use one of them first appear. 
        l: install the package with FORWARD_LOCK
        t: 
    02. Params end when found "]" or "." 

  RULES: install
    01. If detect "r" param, it force reinstall apk
    02. If does not detect "r" param:
        A) If not installed, install it
        B) If installed, only install when version is larger
    03. It install with "f"/"s" params when found

  Install Progress Indicator
    01. It set prop "livall.preapk.count" to report total count
    02. Set prop "livall.preapk.installed" after install every apk
    03. If all apks install complete, "livall.preapk.count"="livall.preapk.installed",
         set prop "ro.preapk.done"=1
    04. Write "1" to "/data/misc/.pre_apk"
    05. If it's not first startup, only set prop "ro.preapk.done"=1

  Uninstall:
    01. Read file "/system/etc/uninstall_apk", one line is one apk package name(such as: cn.livall.TSCalibration)
    02. Every line will unstall
    03. It's not progress indicator

  Args Introduction:
    01. -f
        force do, ignore "/data/misc/.pre_apk"
    02. -ni
        not install, don't execute preapk install
    03. -nu
        not uninstall, don't execute apks uninstall
*/
...
public final class Pm {
    static private final String TAG = "PMJAR";
    static private final String PREAPK_DIR = "/system/etc/.pre_apk";
    static private final String PREAPK_UNINSTALL = "/system/etc/uninstall_apk";
    static private final String FILE_INSTALLED = "/data/misc/.pre_apk";
    static private final String PROP_CNT_TOTAL = "livall.preapk.count";
    static private final String PROP_CNT_INSTALLED = "livall.preapk.installed";
    static private final String PROP_PREAPK_DONE = "ro.preapk.done";
    static private final int APK_CHECK_ERROR = 0x01;
    static private final int APK_NOT_INSTALL = 0x02;
    static private final int APK_VERSION_LESS = 0x03;
    static private final int APK_VERSION_LARGE = 0x04;
    static private final int APK_VERSION_EQUAL = 0x05;
    IPackageManager mPm;
    ...
    private List<PackageInfo> mApkExist;
    private ArrayList<InsAPKInfo> mApkList;
    private ArrayList<String> mApkUninstallList;
    ...
    public void runPreAPK() {
        boolean force = false;
        boolean need_install = true;
        boolean need_uninstall = true;
        ...
        if(!force && isInstalled()) {
            Slog.i(TAG, "PREAPK is installed, ignore ...");
            return;
        }
        Slog.i(TAG, "Now, need_install=" + need_install);
        if(need_install) {
            try {
                mApkExist = getInstalledPackages(mPm, 0);
            } catch (RemoteException e) {
            }
            mApkList = getPreApkList(PREAPK_DIR);

            SystemProperties.set(PROP_CNT_TOTAL, ""+mApkList.size());
            int cnt_installed = 0;
            for(InsAPKInfo apk : mApkList) {
                Slog.i(TAG, "[" + cnt_installed + "] " + apk.name);
                SystemProperties.set(PROP_CNT_INSTALLED, ""+cnt_installed);
                cnt_installed ++;
                if(0!=(apk.flags & PackageManager.INSTALL_REPLACE_EXISTING)) {
                    //force replace install
                    InstallAPK(apk, apk.flags);
                    continue;
                }
                int check = checkApk(apk.name);
                Slog.i(TAG, "Check APP:" + check);
                switch(check) {
                case APK_NOT_INSTALL:
                case APK_VERSION_LARGE:
                    InstallAPK(apk, apk.flags|PackageManager.INSTALL_REPLACE_EXISTING);
                    break;
                }
            }
            SystemProperties.set(PROP_CNT_INSTALLED, ""+mApkList.size());
        }
        SystemProperties.set(PROP_PREAPK_DONE, "1");
        // Now, run uninstall
        Slog.i(TAG, "Now, need_uninstall=" + need_uninstall);
        if(need_uninstall) {
            mApkUninstallList = getUninstallApkList(PREAPK_UNINSTALL);
            if(mApkUninstallList != null) {
                for(String pkg : mApkUninstallList) {
                    UninstallAPK(pkg, 0);
                }
            }
        }
    ...
    private boolean isInstalled() {
        File file = new File(FILE_INSTALLED);
    ...
    private class ScanAPKFilter implements FilenameFilter {
        public boolean accept(File dir, String filename) {            
            Slog.i(TAG, "FIND: " + dir.getAbsolutePath() + "/" + filename);
            final int length = filename.length();
            if(length < 4) return false;
            String extName = filename.substring(length-4);
            return extName.equalsIgnoreCase(".apk");
        }
    }
    private ArrayList<InsAPKInfo> getPreApkList(String path) {
        ArrayList<InsAPKInfo> list = new ArrayList<InsAPKInfo>();
        File file = new File(path);
        if(!file.exists()) {
            Slog.i(TAG, "getPreApkList empty, path not exist.");
            return list;
        }
        File[] files = file.listFiles(new ScanAPKFilter());
        if((files == null) || files.length==0) {
            Slog.i(TAG, "getPreApkList empty.");
            return list;
        }
        for(File f : files) {
            if(f.isDirectory()) continue;
            InsAPKInfo insAPK = new InsAPKInfo(f.getAbsolutePath());
            insAPK.flags = getInsAPKFlag(f.getName());
            list.add(insAPK);
            Slog.i(TAG, "ADD: " + insAPK.name);
        }
        return list;
    }

    private int getInsAPKFlag(String name) {
        if(name.charAt(0) != '[') return 0;
        if(name.charAt(1) != 'c' || name.charAt(1) != 'C') return 0;
        int installFlags = 0;
        final int length = name.length() - 3;
    ...
    private int checkApk(String apkPathName) {
        PackageInfo find = null;
        PackageInfo pinfo = null;
        Slog.i(TAG, "checkApp : " + apkPathName);

        PackageParser packageParser = new PackageParser(apkPathName);  
        File sourceFile = new File(apkPathName);  
        DisplayMetrics metrics = new DisplayMetrics();  
        metrics.setToDefaults();  
        PackageParser.Package pkg = packageParser.parsePackage(sourceFile, apkPathName, metrics, 0);  
        if (pkg == null) {  
            return APK_CHECK_ERROR;  
        }   
        for(PackageInfo i : mApkExist) {
            if(i.packageName.equals(pkg.packageName)) {
                find = i;
                break;
            }
        }
        if(find == null) return APK_NOT_INSTALL;
        if(pkg.mVersionCode == find.versionCode) return APK_VERSION_EQUAL;
        if(pkg.mVersionCode > find.versionCode) return APK_VERSION_LARGE;
     ...
     * 
     */
    private class InsAPKInfo {
        InsAPKInfo(String _name) {
            flags = 0;
            name = _name;
        }
        int flags;
    ...
    private boolean InstallAPK(InsAPKInfo apk, int flags) {
        final Uri apkURI;
        final Uri verificationURI = null;
        apkURI = Uri.fromFile(new File(apk.name));
        PackageInstallObserver obs = new PackageInstallObserver();        
        try {
            mPm.installPackageWithVerification(apkURI, obs, flags, null,
                    verificationURI, null);
            ...
            synchronized (obs) {
                ...
                if (obs.result == PackageManager.INSTALL_SUCCEEDED) {
                    Slog.i(TAG, "Install [" + apk.name + "] Success");
                    return true;
                } else {
                    Slog.i(TAG, "Failure ["
                            + installFailureToString(obs.result)
    ...
    private ArrayList<String> getUninstallApkList(String filename) {
    ...
    private boolean UninstallAPK(String pkg, int unInstallFlags) {
        Slog.i(TAG, "UninstallAPK: " + pkg);
