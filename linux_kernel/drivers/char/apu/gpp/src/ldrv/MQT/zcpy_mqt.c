...
#if defined(USE_RING_MSGQ)
STATIC PoolId g_poolId = 0;
STATIC Uint16 g_msgSize = 0;
#endif
...
ZCPYMQT_open (IN  LDRV_MSGQ_TransportHandle mqtHandle, IN  Pvoid mqtAttrs)
{
    ...
    if (DSP_SUCCEEDED (status)) {
#if defined(USE_RING_MSGQ)
        RMSGQ_init((void*)&ctrlPtr->toDspQueue);
		RMSGQ_init((void*)&ctrlPtr->fmDspQueue);
#else
...
ZCPYMQT_close (IN  LDRV_MSGQ_TransportHandle mqtHandle)
{
    ...
    if (zcpyMqtState != NULL) {
        ...
        if (ctrlPtr != NULL) {
            ...
            if ( (RMSGQ_isEmpty ((void*)&ctrlPtr->toDspQueue) == FALSE)
                   && (DSP_SUCCEEDED (status))) {
                LDRV_MPCS_enter ((MPCS_ShObj *) &(ctrlPtr->csToDspList)) ;
                RMSGQ_clear((void*)&ctrlPtr->toDspQueue);
                LDRV_MPCS_leave ((MPCS_ShObj *) &(ctrlPtr->csToDspList)) ;
            }

            if ( (RMSGQ_isEmpty ((void*)&ctrlPtr->fmDspQueue) == FALSE)
                   && (DSP_SUCCEEDED (status))) {
                LDRV_MPCS_enter ((MPCS_ShObj *) &(ctrlPtr->csFmDspList)) ;
                RMSGQ_clear((void*)&ctrlPtr->fmDspQueue);
                LDRV_MPCS_leave ((MPCS_ShObj *) &(ctrlPtr->csFmDspList)) ;
            }
#else
            /* Clear out any messages not yet processed */
            while ( (LDRV_MPLIST_isEmpty (dspId, (List *) &(ctrlPtr->toDspList)) == FALSE)
                   && (DSP_SUCCEEDED (status))) {
                LDRV_MPCS_enter ((MPCS_ShObj *) &(ctrlPtr->csToDspList)) ;
    ...
    while (   (DSP_SUCCEEDED (status))
#if defined(USE_RING_MSGQ)
           && !RMSGQ_isEmpty ((void*)&ctrlPtr->fmDspQueue)) {
        LDRV_MPCS_enter ((MPCS_ShObj *) &(ctrlPtr->csFmDspList)) ;
        LDRV_MSGQ_alloc(g_poolId, g_msgSize, &recvMsg);
        RMSGQ_get((void*)&ctrlPtr->fmDspQueue, recvMsg, recvMsg->size);
        tPoolId = recvMsg->dstId; //poolId as temp value
        recvMsg->dstId = recvMsg->srcId;
        recvMsg->srcId = tPoolId;
#else
        ...
        if (recvMsg != NULL) {
            ...
            if (recvMsg->dstId == MSGQ_INVALIDMSGQ) {
                    ...
                    if (    msg->ctrlMsg.locateAckMsg.replyQueue
                        ...
                        if (msgqQueue != (Uint32) MSGQ_INVALIDMSGQ) {
                            ...
                            status = LDRV_MSGQ_alloc (
                                     tPoolId,
                                     APUDRV_ALIGN (sizeof(MSGQ_AsyncLocateMsg),
                                                    APUDRV_BUF_ALIGN),
                ...
                APUDRVLOG_LogEvent (DG_MSGQ_QUE,
                                     recvMsg->msgId,
                                     (Uint32) msgqQueue,
                                     (Uint32) recvMsg,
                                     recvMsg->size,
              ...
              IN Pvoid            msg)
{
    ...
    LDRV_MPCS_enter ((MPCS_ShObj *) &(ctrlPtr->csToDspList)) ;
#if defined(USE_RING_MSGQ)
    g_poolId  = ((MSGQ_Msg)msg)->poolId;
    g_msgSize = ((MSGQ_Msg)msg)->size;
    while(RMSGQ_SUCCESS != RMSGQ_put((void*)&ctrlPtr->toDspQueue, msg, ((MSGQ_Msg)msg)->size)) {
        DPC_msleep(1, 1);
    }
    LDRV_MSGQ_free((MSGQ_Msg)msg);
#else
