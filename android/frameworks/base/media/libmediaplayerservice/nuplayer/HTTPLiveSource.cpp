    ...
    for (int32_t i = 0; i < 50; ++i) {
        ...
        if (n == -EWOULDBLOCK) {
            ...
            if (buffer[0] == 0x00) {
                ...
                /* a@nufront start */
                LCHLOGD1("preroll=, buffer[1]=0x%x, buffer[2]=0x%x", buffer[1], buffer[2]);
                if ((0x02 == buffer[2]) || (0x03 == buffer[2])) {
                    int64_t mediatimeUs;
                    memcpy((void*) &mediatimeUs, &buffer[3], sizeof(mediatimeUs));
                    ...
                    if(0x03 == buffer[2]) {
                        extra->setInt64("jump-at-prerollUs", mediatimeUs);
                    } else {
                    extra->setInt64("resume-at-prerollUs", mediatimeUs);
                    }
