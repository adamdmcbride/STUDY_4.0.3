...
AudioSource::AudioSource(
    ...
    mRecord = new AudioRecord(
                ...
                /*m@nufront start: extend ringbuf to avoid audioflinger overflow*/
                //4 * kMaxBufferSize / sizeof(int16_t), /* Enable ping-pong buffers */
                40 * kMaxBufferSize / sizeof(int16_t), /* Enable ping-pong buffers */
                /*m@nufront end*/
                flags,
...
void AudioSource::waitOutstandingEncodingFrames_l() {
    ...
    /*m@nufront start: wait until all buffer have been handled*/
    while (mNumClientOwnedBuffers > 0 || !mBuffersReceived.empty()) {
    /*m@nufront end*/
        LOGD("will mFrameEncodingCompletionCondition.wait(mLock), mNumClientOwnedBuffers is %lld, mBuffersReceived.size() is %d", mNumClientOwnedBuffers, mBuffersReceived.size());
        mFrameEncodingCompletionCondition.wait(mLock);
        LOGD("after mFrameEncodingCompletionCondition.wait(mLock)");
    }
}
...
status_t AudioSource::stop() {
    ...
    LOGD("after set mStarted false for AudioSource");
    /*a@nufront end*/
    mRecord->stop();
    LOGD("after mRecord->stop()");
...
status_t AudioSource::read(
    ...
    *out = NULL;
    ...
    //    return OK;
    //}
    if (!mStarted && mBuffersReceived.empty()) {
        ZJFLOGD("function out for !mStarted &&mBuffersReceived.empty()");
        return NO_INIT;
    }
    /*m@nufront end*/
...
status_t AudioSource::dataCallbackTimestamp(
    ...
    /*m@nufront start: need not sleep so long*/
    {
        Mutex::Autolock autoLock(mLock);
