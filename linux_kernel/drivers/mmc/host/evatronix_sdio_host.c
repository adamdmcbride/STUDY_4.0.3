...
/**************************************************************************/
extern struct mmc_host *bcm_slot2_mmc = NULL;
...
struct evatronix_slot {
	...
	unsigned int 		boundary;	// DMA buffer boundary
...
struct evatronix_host {
	...
	unsigned long long 	queued_reqs;
	unsigned int		flag;
#ifdef DEBUG
	u32			prev_slot;
	u32 			prev_cmd;
...
static void soft_reset(struct evatronix_host *host, struct evatronix_slot *slot, int flag)
{
	...
	if(flag == SRCMD) {
		...
		if(val & 0xF00000) {
			val &= ~0xF8000;
			val |= 0xF80000;
			slot_writel(host, SFR12, slot->id, val);
		}
		...
		struct evatronix_slot *slot, struct mmc_request *mrq)
{
	...
	if(cmd->error ) {
       ...
       if(cmd->error && (cmd->opcode != 5)
                       ...
                       && (cmd->opcode != 55)) {
#endif
		err("slot%u: CMD%u %d, req[%lu]", slot->id, cmd->opcode, cmd->error, slot->reqid);
#ifdef DEBUG
		evatronix_dbg(slot->mmc);
#endif
	}
	...
	if ( (!cmd->error)  && (cmd->flags & MMC_RSP_PRESENT) ) {
		if (cmd->flags & MMC_RSP_136) {
			...
			VDBG("slot%u: RESP 0x%08x 0x%08x 0x%08x 0x%08x", id,
			...
			VDBG("slot%u: RESP 0x%08x", id, cmd->resp[0]);
		}
	}
	...
	if(data) {
		if(data->error) {
			dataError = 1;
			err("slot%u: CMD%d DAT %s error %d, irq_status 0x%08x",
					id, cmd->opcode,
	...
	if(dataError) {
		if(slot->ctype == SDIO_CARD) {
			err("Damn! Try to reset SDIO");
			spin_unlock(&host->lock);
			/* redemption, maybe should destroy the queue */
			struct list_head *l;
			err("dequeue all reqs");
			list_for_each(l, &host->queue) {
				new_slot = container_of(l, struct evatronix_slot, queue_node);
				new_slot->mrq->cmd->error = -EBUSY;
				list_del(&new_slot->queue_node);
				if(new_slot->mrq)
					mmc_request_done(new_slot->mmc, new_slot->mrq);
			}

			/* reborn */
			bcm_wlan_power_on(0);
			//extern int sdio_reset_comm(struct mmc_card *card);
			//sdio_reset_comm(slot->mmc->card);
		...
		struct evatronix_slot *slot, struct mmc_request *req)
{
	...
	if(host->state == STATE_IDLE) {
		...
		VDBG("slot%u: host BUSY %x, queued_reqs %lu",
				slot->id, host->state, host->queued_reqs++);
...
//	host->prev_cmd  = 0xff;
	...
	if(s3_test)
		err("slot%u: CMD%d SFR2 0x%08x, SFR3 0x%08x",
				slot->id, cmd->opcode,
...
static void evatronix_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if(ios->clock) {
		...
		eng("slot%d: source rate %d, divider %d, ios->clock %d", slot->id, source_rate, div, ios->clock);
...
static int evatronix_slot_get_cd(struct evatronix_host *host, struct evatronix_slot *slot)
{
	...
	if((slot->ctype == SDIO_CARD) || (slot->ctype == EMMC_CARD) ) {
		LEAVE();
		return 0xF;
	}
...
static int evatronix_get_cd(struct mmc_host *mmc, struct mmc_ios *ios)
{
...

	{
		 ...
		 * so just return 1, do force scan, but maybe crash
		 * kernel when there's no SDIO-capable/eMMC-capable device
		 * present at this kind of slot.
		 */
		if( (reg&(1<<16/*card inserted*/)) || (slot->ctype == EMMC_CARD)
				|| (slot->ctype == SDIO_CARD) ) {
...
static int evatronix_create_slot(struct evatronix_host *host, int id)
{
	...
	slot->bus_width = pdata->bus_width;
	slot->freq	= pdata->freq;
	slot->ocr_avail = pdata->ocr_avail;
	...
	err("slot%u: max_blk_size 0x%x", id, mmc->max_blk_size);
	...
	eng("slot%u: ocr val 0x%x, voltage range 0x%x", slot->id, slot->ocr_avail, slot->voltage);

	switch(slot->bus_width) {
		case 4:
	...
	/* 1G/2G std card supportted */
	mmc->caps |= (/*MMC_CAP_SD_HIGHSPEED |*/MMC_CAP_MMC_HIGHSPEED);
	if(SDIO_CARD == slot->ctype) {
		mmc->caps |= MMC_CAP_SDIO_IRQ;
		mmc->caps |= MMC_CAP_NONREMOVABLE;
		mmc->pm_caps |= MMC_PM_KEEP_POWER;
		//mmc->pm_caps |= MMC_PM_WAKE_SDIO_IRQ;
		mmc->pm_caps |= MMC_PM_IGNORE_PM_NOTIFY;
		printk(KERN_ALERT "slot%u: pm_caps is 0x%x\n", slot->id, mmc->pm_caps);
	}

	printk(KERN_ALERT "slot%u: caps is 0x%x\n", slot->id, mmc->caps);

	evatronix_init_slot(host, id);

#ifdef CONFIG_BCMDHD
	if((slot->ctype == SDIO_CARD) && (slot->id == SDIO_SLOT_ID) )
		bcm_slot2_mmc = mmc;
#endif
...
static void evatronix_init_slot(struct evatronix_host *host, int id)
{
	...
	//SFR1_DMA_BUFF_SIZE_512KB
	host->slots[id]->boundary = 0x00007000;
...
static int evatronix_probe(struct platform_device *pdev)
{
	...
	printk(KERN_EMERG "evatronix sdmmc/sdio IRQ %d", irq);
	...
	ret = clk_set_rate(host->fclk, CLK_FREQ);
	if(ret)
	...
	host->gpio		= pdata->gpio;
	printk(KERN_INFO "host->gpio %d, pdata->gpio %d\n", host->gpio, pdata->gpio);
	...
	printk(KERN_INFO "evatronix sdio host paddr base 0x%08x", host->phys_base);

	host->virt_base = ioremap(res->start, res->end - res->start + 1);
	printk(KERN_INFO " vaddr base 0x%08x\n", host->virt_base);
...
#ifdef CONFIG_BCMDHD
#ifdef CONFIG_MACH_NS115_PAD_REF
		host->regu = regu = regulator_get(NULL, "vdd_wifi_1v8");
		if(IS_ERR(regu)) {
			err("%s: regulator_get failed", __func__);
			ret = PTR_ERR(regu);
		//	goto out;
		} else  {
			regulator_enable(host->regu);
			mdelay(10);
		}
#endif
		/* both testboard & refboard need this */
		bcm_wlan_power_on(0);
#endif
	}
	...
	for (i = 0; i < host->nr_slots; i++) {
		if(slot_readl(host, SFR9, i) & 0x10000) {
			printk(KERN_INFO "slot%u: card present", i);
		}
...
{
	//platform_device_unregister(&ns2815_sdmmc_device);
