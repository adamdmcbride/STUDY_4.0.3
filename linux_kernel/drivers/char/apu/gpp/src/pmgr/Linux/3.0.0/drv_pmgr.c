 ...
 *  @file   drv_pmgr.c
 *
 *  @path   $(APUDRV)/gpp/src/pmgr/Linux/3.0.0/
 *
 ...
 *  ----------------------------------------------------------------------------
 */
STATIC Int32 major = 0 ;

/** ----------------------------------------------------------------------------
 *  @name   minor
 *
 *  @desc   Minor number of driver.
 *  ----------------------------------------------------------------------------
 */
STATIC Int32 first_minor = 0 ;
 ...
 *  ----------------------------------------------------------------------------
 */
STATIC struct class *apudrv_class;

/** ----------------------------------------------------------------------------
 *  @name   apudrv_cdev
 *
 *  @desc   CDEV_t struct for device create.
 *  ----------------------------------------------------------------------------
 */
STATIC struct cdev  apudrv_cdev;
...
int __init DRV_InitializeModule (void)
{
    ...
    PRINT_Printf ("APUDRV Module (%s) created on Date: %s Time: %s\n",
                  APUDRV_VERSION,
                  __DATE__,
                  __TIME__) ;
    ...
    if (DSP_SUCCEEDED (status)) {
        ...
        if (major) {
            ...
            result = register_chrdev_region (dev, minor_count, "apudrv");
            if (result < 0) {
                status = DSP_EFAIL ;
                SET_FAILURE_REASON ;
                TRC_1PRINT (TRC_LEVEL7,
                            "Linux API register_chrdev_region returned error: %d\n",
                            result) ;
            }
        } else {
            result = alloc_chrdev_region (&dev, first_minor, minor_count, "apudrv");
            if (result < 0) {
                status = DSP_EFAIL ;
                SET_FAILURE_REASON ;
                TRC_1PRINT (TRC_LEVEL7,
                            "Linux API alloc_chrdev_region returned error: %d\n",
                            result) ;
            }
            major = MAJOR (dev);
        ...
        cdev_init (&apudrv_cdev, &driverOps);
        apudrv_cdev.owner = THIS_MODULE;
        result = cdev_add (&apudrv_cdev, dev, 1);
        if (result < 0) {
            status = DSP_EFAIL ;
            SET_FAILURE_REASON ;
            TRC_1PRINT (TRC_LEVEL7,
                        "Linux API cdev_add returned error: %d\n",
                        result) ;
        }
#else
        result = register_chrdev (major, "apudrv", &driverOps) ;
        if (result < 0) {
            status = DSP_EFAIL ;
            SET_FAILURE_REASON ;
            TRC_1PRINT (TRC_LEVEL7,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
        apudrv_class = class_create (THIS_MODULE, "apudrv");
        device_create (apudrv_class, NULL, MKDEV(major, 0), NULL, "apudrv");
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
    device_destroy(apudrv_class, MKDEV(major, 0));
    class_destroy(apudrv_class);
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
    dev = MKDEV(major, first_minor);
    cdev_del(&apudrv_cdev);
    unregister_chrdev_region(dev, minor_count);
#else
    unregister_chrdev (major, "apudrv") ;
#endif
    ...
    case CMD_MSGQ_PUT:
        {
            ...
            /* Log the event */
            APUDRVLOG_LogEvent (GD_MSGQ_PUT,
...
APUDRV_Cleanup (Void)
{
    ...
    for (i = 0 ; i < DRV_MaxMsgqs ; i++) {
        ...
        status = PMGR_MSGQ_close (msgqQueue, &isForce) ;
