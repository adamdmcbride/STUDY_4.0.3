...
struct eva_i2s_pridata {
	...
	u32 dev_conf;
	u32 fmt;
	struct delayed_work work_trigger;
	atomic_t work_ing;
	atomic_t trigger_mute;
	void __iomem * base;
};
struct hdmi_params {
	u32 sample_rate;
...
#endif
extern void rt5631_do_trigger_work(int mute);
...
static void nusmart_do_trigger_work(struct work_struct *work)
{
	if(atomic_read(&eva_i2s_data.trigger_mute)) {
		rt5631_do_trigger_work(1);
	} else {
		rt5631_do_trigger_work(0);
	}
	atomic_set(&eva_i2s_data.work_ing, 0);
}

static void nusmart_codec_mute(int mute, int force_do)
{
//	if(codec_mode == INCALL) mute = 0;
//	if(codec_hdmi_mute) mute = 1;
	if(force_do || atomic_read(&eva_i2s_data.trigger_mute) != mute) {
		atomic_set(&eva_i2s_data.trigger_mute, mute);
		if(!atomic_read(&eva_i2s_data.work_ing)) {
			atomic_set(&eva_i2s_data.work_ing, 1);
			schedule_delayed_work(&eva_i2s_data.work_trigger, mute?(10*HZ):0);
		}
	}
}
		...
		struct snd_soc_dai *dai)
{
	...
	int ret = 0;
	int mute = atomic_read(&eva_i2s_data.trigger_mute);
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int eva_i2s_probe(struct platform_device *dev)
{
	...
	DBG_PRINT("%s ....\n", __func__);
	INIT_DELAYED_WORK(&eva_i2s_data.work_trigger, nusmart_do_trigger_work);
