...
public class TelephonyManager {
    ...
    public String getDeviceId() {
        try {
//nufront@start
		String id=getSubscriberInfo().getDeviceId();	
		//Get a null id cause we don't have an IMEI.Then we have to ask imei.conf for a id;
		if(id == null){
		       System.out.println("Reading IMEI from file /system/etc/imei.conf");
		       try{
				FileReader imei_file = new FileReader("/system/etc/imei.conf");
				
				BufferedReader br = new BufferedReader(imei_file);
				id = br.readLine();
				System.out.println("IMEI from config file:"+id);	
				br.close();
				imei_file.close();
				return id;
			}
			catch(IOException e){
				System.out.println("Read config file error.Use default IMEI=862502410353683;");	
				return "862502410353683"; 
			}	
	       }else{
			System.out.println("Get device id in success!");
			 return id;
		}
//nufront@end
