...
static void *readerLoop(void *arg)
{
    for (;;) {
		...
		if(strStartsWith(line,"+STKPRO:") && !strStartsWith(line,"+STKPRO: \"D") ){
			LOGD("readerLoop has more str----------continue-------------");
			continue;
		}
/*
		char * temp = line;
		...
		err = at_tok_nextstr(&temp, &temp1);

		if(at_tok_hasmore(&temp)){
			LOGD("readerLoop has more str-----------------------");
			break;
		}
*/
