...
#define MIC_POWER_M_ADD 0x3B
#define MIC2_POWER_M_VALUE 0xfc16
#define MIC1_POWER_M_VALUE 0xfc2a
...
struct snd_soc_codec *rt5631_codec;
...
void rt5631_do_trigger_work(int mute)
{
	unsigned int val = snd_soc_read(rt5631_codec,RT5631_PWR_MANAG_ADD1);
	if(!mute)
		val |= (RT5631_PWR_CLASS_D);
	else 
		val &= ~(RT5631_PWR_CLASS_D);
	snd_soc_write(rt5631_codec,RT5631_PWR_MANAG_ADD1,val);
}
EXPORT_SYMBOL(rt5631_do_trigger_work);
...
int rt5631_mic_gpio_check(int hp_insert)
{
	unsigned int reg = 0;
	int mic;
	if(hp_insert == 0){//insert headset
		reg = snd_soc_read(rt5631_codec,MIC_POWER_M_ADD);
		if(!((reg & MIC2_POWER_M_VALUE)==MIC2_POWER_M_VALUE)) {
			snd_soc_write(rt5631_codec,RT5631_PWR_MANAG_ADD3,snd_soc_read(rt5631_codec,RT5631_PWR_MANAG_ADD3) | (1 << 13));
			snd_soc_write(rt5631_codec, MIC_POWER_M_ADD, reg|MIC2_POWER_M_VALUE);
			msleep(60);
			mic = gpio_get_value(GPIOA(2));
			snd_soc_write(rt5631_codec, MIC_POWER_M_ADD, reg);
		} else {
			mic = gpio_get_value(GPIOA(2));
		}
...
EXPORT_SYMBOL(rt5631_mic_gpio_check);

static const char *rt5631_mic_sel_mode[] = {"mic1", "mic2"};
static const SOC_ENUM_SINGLE_DECL(rt5631_mic_sel_enum, 0, 0, rt5631_mic_sel_mode);
static int rt5631_mic_sel_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
	...
	if(val == 1)
	{
		snd_soc_dapm_disable_pin(dapm, "Mic Jack1");
		snd_soc_dapm_enable_pin(dapm, "Mic Jack2");
	}
	else
	{
		snd_soc_dapm_enable_pin(dapm, "Mic Jack1");
		snd_soc_dapm_disable_pin(dapm, "Mic Jack2");
	}
	mic_sel = val;
}

static int rt5631_mic_sel_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct soc_mixer_control *mc =
		(struct soc_mixer_control *)kcontrol->private_value;
...
static const struct snd_kcontrol_new rt5631_snd_controls[] = {
	...
	SOC_DOUBLE_TLV_SCALE("PCM Playback Volume", RT5631_STEREO_DAC_VOL_2,
			RT5631_L_VOL_SHIFT, RT5631_R_VOL_SHIFT,
			0, RT5631_DAC_VOL_MASK, 1, dac_vol_tlv),
...
#endif
	SOC_ENUM_EXT("MIC SEL Switch", rt5631_mic_sel_enum,
			rt5631_mic_sel_get, rt5631_mic_sel_put),
