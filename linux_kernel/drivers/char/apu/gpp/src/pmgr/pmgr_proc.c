...
PMGR_PROC_setup (IN LINKCFG_Object * linkCfg)
{
    ...
    /*  ------------------------------------------------------------------------
     *  First caller sets up the APUDRV driver.
     *  ------------------------------------------------------------------------
     */
    if (toSetup == TRUE) {
...
PMGR_PROC_destroy (Void)
{
    ...
    /*  ------------------------------------------------------------------------
     *  Last caller finalizes the APUDRV driver.
     *  ------------------------------------------------------------------------
     */
    if (toDestroy == TRUE) {
        for (i = 0 ; i < MAX_DSPS ; i++) {
    ...
    if (DSP_SUCCEEDED (status)) {
        if (toLoad == TRUE) {
            ...
            if (DSP_SUCCEEDED (status)) {
                 ...
                 */
                /* for zsp if loaderIntf is NULL, direct copy duguiyu 2012-03-09 */
                if (loaderIntf == NULL) {
                    //status = DSP_EFAIL ;
                    //SET_FAILURE_REASON ;
#if 1
                    ...
                    kfileIntf->kfileOpen( imagePath, "rb", &fileHandle);
				    kfileIntf->kfileSeek( fileHandle, 0, 2);  //SEEK_END
					kfileIntf->kfileTell( fileHandle, &fileSize);
					kfileIntf->kfileSeek( fileHandle, 0, 0);  //SEEK_SET
					if(fileSize != 0x30000)
					{
						status = DSP_EFAIL ;
                        SET_FAILURE_REASON ;
                    ...
                    kfileIntf->kfileRead((char*)memTable[dspObj->arg1].gppVirtAddr, 
                               dspObj->maduSize, memTable[dspObj->arg1].size, fileHandle);

					kfileIntf->kfileRead((char*)memTable[dspObj->arg2].gppVirtAddr, 
					           dspObj->maduSize, memTable[dspObj->arg2].size, fileHandle);
					kfileIntf->kfileClose( fileHandle );
#endif
                    ...
                    if (DSP_FAILED (status)) {
                        ...
                        if (status == DSP_ERANGE) {
                            PRINT_Printf (
                                     ...
                                     "/apudrv/config/all/CFG_<PLATFORM>.c\n") ;
                        }
                    }
                }
                else {
