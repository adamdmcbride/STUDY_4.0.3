...
/*********************************************/
static inline bool audio_is_wire_headset_device(audio_devices_t device)
{
    if ((popcount(device) == 1) && (device & AUDIO_DEVICE_OUT_WIRED_HEADSET))
        return true;
    else
        return false;
}

static inline bool audio_is_hdmi_audio_device(audio_devices_t device)
{
