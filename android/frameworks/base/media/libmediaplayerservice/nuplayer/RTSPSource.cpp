...
void NuPlayer::RTSPSource::stop() {
    /* a@nufront start , unnecessary to send msg if disconnected state*/
    LCHLOGD1("mState=%d", mState);
    if(DISCONNECTED == mState) {
        return;
    }
    /* a@nufront end */
...
status_t NuPlayer::RTSPSource::getDuration(int64_t *durationUs) {
    *durationUs = 0ll;
    ...
    LCHLOGD1("audio %f s, video %s", (audioDurationUs/1000000.0), (videoDurationUs/1000000.0));
    return OK;
}
