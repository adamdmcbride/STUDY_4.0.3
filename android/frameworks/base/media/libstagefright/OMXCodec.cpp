...
#include "nufront/ffmpeg_extractor/FFmpegUtils.h"
...
/*a@nufront start*/
static const int OMX_VPU_RV8_VERSION = 0x30200000;
static const int OMX_VPU_RV9_VERSION = 0x40000000;
/*a@nufront end*/
...
static const CodecInfo kDecoderInfo[] = {
    ...
    { MEDIA_MIMETYPE_AUDIO_MPEG, "OMX.NU.APU.codec" },
#else
    { MEDIA_MIMETYPE_AUDIO_MPEG, "OMX.google.mp3.decoder" },
#endif
#ifdef WITH_AAC_APU_CODEC
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.NU.APU.codec" },
#else
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.google.aac.decoder" },
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.NU.Audio.Decoder" },
#endif
#else
    { MEDIA_MIMETYPE_AUDIO_MPEG, "OMX.google.mp3.decoder" },
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.google.aac.decoder" },
...
//    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.Nvidia.aac.decoder" },
    { MEDIA_MIMETYPE_AUDIO_NUAUDIO, "OMX.NU.Audio.Decoder" },
...
#if VPU_SUPPORT
    { MEDIA_MIMETYPE_VIDEO_MPEG4, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_H263, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_MPEG1, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_MPEG2, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_FLV1, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_VP6, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_VP6F, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_VP6A, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_VC1, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_RV30, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_RV40, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_VP8, "OMX.NU.Video.Decoder" },
    { MEDIA_MIMETYPE_VIDEO_MJPEG, "OMX.NU.Video.Decoder" },
#else
...
/*a@nufront: start*/
#if VPU_SUPPORT && NS115
    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.NU.Video.Encoder" },
    { MEDIA_MIMETYPE_VIDEO_VP8, "OMX.NU.Video.Encoder" },
    { MEDIA_MIMETYPE_IMAGE_JPEG, "OMX.NU.Video.Encoder" },
#else
...
#define CODEC_LOGI(x, ...) LOGI("%d [%s] "x,  __LINE__, mComponentName, ##__VA_ARGS__)
#define CODEC_LOGV(x, ...) LOGV("%d [%s] "x, __LINE__, mComponentName, ##__VA_ARGS__)
#define CODEC_LOGE(x, ...) LOGE("%d [%s] "x, __LINE__, mComponentName, ##__VA_ARGS__)
...
#ifdef WITH_APU_CODEC
    else if (!strcmp(componentName, "OMX.NU.APU.codec")) {
        quirks |= kRequiresAllocateBufferOnInputPorts;
        quirks |= kRequiresAllocateBufferOnOutputPorts;
        quirks |= kNeedsFlushBeforeDisable;
        quirks |= kDecoderLiesAboutNumberOfChannels;
...
/*a@nufront: start*/
#if VPU_SUPPORT
    if (!strcmp(componentName, "OMX.NU.Video.Decoder")) {
        quirks |= kNeedsFlushBeforeDisable;
        quirks |= kInputBufferSizesAreBogus;
        quirks |= kRequiresAllocateBufferOnInputPorts;
        quirks |= kRequiresAllocateBufferOnOutputPorts;
    }
    if (!strcmp(componentName, "OMX.NU.Video.Encoder")) {
        //quirks |= kRequiresLoadedToIdleAfterAllocation;
        //quirks |= kRequiresLargerEncoderOutputBuffer;
        //quirks |= kAvoidMemcopyInputRecordingFrames;
        quirks |= kRequiresAllocateBufferOnInputPorts;
...
void OMXCodec::findMatchingCodecs(
    ...
    LCHLOGV("mime = %s, flags = 0x%x", mime, flags);
    for (int index = 0;; ++index) {
        ...
        if (((flags & kSoftwareCodecsOnly) &&   IsSoftwareCodec(componentName)) ||
            ...
            LCHLOGV("mime = %s, componentName = %s", mime, componentName);
...
sp<MediaSource> OMXCodec::Create(
    ...
    CHECK(success);
    LCHLOGD1("findMatchingCodecs flags = 0x%x", flags);

    //soft AAC decoder only supports 2 channels, so use ffmpeg aac decoder if multi-channle source
    if(!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mime))
    {
        int32_t numChannels = 0x0;
        if(meta->findInt32(kKeyChannelCount, &numChannels) && (numChannels > 0x2))
        {
            FFmpegUtils::RegisterCodec((char* ) mime);
...
/*a@nufront: start*/
/*set dvfs governor to PERFORMANCE when video resolution is larger than 800*600(temporary)*/
#ifdef GOVERNOR_CTRL
                if (!OMXCodec::isDvfsFlag && !codec->mIsEncoder) {
                    if((codec->mFrameWidth * codec->mFrameHeight) >= 800*600) {
                        /*change DFVS gover*/
                        if(!readGovernor()) {
                             OMXCodec::initDvfsCnt++;
                             OMXCodec::isDvfsFlag = 1;
                             property_set(DVFS_GOVERNOR_KEY, DVFS_GOVERNOR_PERFORMANCE);
                             LOGV("Set Current DVFS Governor(Performance)");
                        }
                     }
                }
                else if (OMXCodec::isDvfsFlag && !codec->mIsEncoder
                      && !strcmp("OMX.NU.Video.Decoder", componentName)) {
                    if((codec->mFrameWidth * codec->mFrameHeight) >= 800*600) {
                        OMXCodec::initDvfsCnt++;
                    }
                }
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        if (meta->findData(kKeyESDS, &type, &data, &size)) {
            ...
            /*m@nufront start: add for support mp3 in mp4*/
            if (0 != codec_specific_data_size) {
            ...
            */
            ...
            addCodecSpecificData(data, size);
        /*a@nufront start*/
        } else if (meta->findData(kKeyMPEG4InMKV, &type, &data, &size)) {
            addCodecSpecificData(data, size);
        /*a@nufront end*/
        }
    }
...
/*a@nufront: end*/
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_FLV1, mMIME)) {
        const char *mpeg4Type;
        meta->findCString(kKeyMPEG4, &mpeg4Type);
        int32_t width, height;
        meta->findInt32(kKeyWidth, &width);
        meta->findInt32(kKeyHeight, &height);
        setMpeg4Format(width, height, MP4DEC_SORENSON);
    }
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VC1, mMIME)) {
        int32_t width, height;
        meta->findInt32(kKeyWidth, &width);
        meta->findInt32(kKeyHeight, &height);
        const char *vc1Type;
        if(meta->findCString(kKeyVC1Type, &vc1Type)){
            if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VC1, vc1Type)) {
                setVC1Format(width, height, VC1DEC_VC1);
            } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VC1_WMV3, vc1Type)) {
                setVC1Format(width, height, VC1DEC_WMV3);
            }
        }
        else{
            setVC1Format(width, height, VC1DEC_VC1);
    ...
    LCHLOGD1("mMIME = %s", mMIME);
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP6, mMIME)
            || !strcasecmp(MEDIA_MIMETYPE_VIDEO_VP6F, mMIME)
            || !strcasecmp(MEDIA_MIMETYPE_VIDEO_VP6A, mMIME)) {
        setVP6Format(!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP6, mMIME));
    }
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV40, mMIME)) {
        int32_t width, height;
        meta->findInt32(kKeyWidth, &width);
        meta->findInt32(kKeyHeight, &height);
        setRVFormat(width, height, OMX_VPU_RV9_VERSION);
    }
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV30, mMIME)) {
        int32_t width, height;
        meta->findInt32(kKeyWidth, &width);
        meta->findInt32(kKeyHeight, &height);
        setRVFormat(width, height, OMX_VPU_RV8_VERSION);
    }
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4, mMIME)) {
        const char *mpeg4Type;
        if(meta->findCString(kKeyMPEG4, &mpeg4Type)){
            if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4, mpeg4Type)) {
                setMpeg4Format(0, 0, MP4DEC_MPEG4);
            } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4_DIVX, mpeg4Type)) {
                setMpeg4Format(0, 0, MP4DEC_CUSTOM_1);
            } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4_DIVX3, mpeg4Type)) {
                int32_t width, height;
                meta->findInt32(kKeyWidth, &width);
                meta->findInt32(kKeyHeight, &height);
                setMpeg4Format(width, height, MP4DEC_CUSTOM_1);
            }
        }
        else{
            setMpeg4Format(0, 0, MP4DEC_MPEG4);
...
/*a@nufront end*/
    ...
    if (!strncasecmp(mMIME, "video/", 6)) {
        ...
        if (mIsEncoder) {
            ...
            LCHLOGD1("mMIME = %s, success=%d, width=%d, height=%d", mMIME, (int) success, width, height);
            CHECK(success);
    ...
    /*a@nufront start: add to pass codecCtx to component*/
    if (!strcmp(mComponentName, "OMX.NU.Audio.Decoder")) {
        int32_t err;
        err = setNuAudioDecoder(meta);
        if (err != OK) {
            return err;
        }
    }
#ifdef WITH_APU_CODEC
    if (!strcmp(mComponentName, "OMX.NU.APU.codec")) {
        int32_t err;
        err = setNuApuCodec(meta);
        if (err != OK) {
            return err;
        }
    }
...
/*a@nufront start*/
void OMXCodec::setMpeg4Format(int32_t width, int32_t height, MP4DecStrmFmt fmt) {
    OMX_U32 portIndex = mIsEncoder ? kPortIndexOutput : kPortIndexInput;
    OMX_VIDEO_PARAM_MPEG4TYPE def;
    def.nPortIndex = portIndex;
    def.nMP4DecStrmFmt = fmt;
    def.nWidth = width;
    def.nHeight = height;
    status_t err = mOMX->setParameter(mNode, OMX_IndexParamVideoMpeg4, &def, sizeof(def));
    CHECK_EQ(err, (status_t)OK);
}

void OMXCodec::setRVFormat(int32_t width, int32_t height, int32_t version) {
    OMX_U32 portIndex = mIsEncoder ? kPortIndexOutput : kPortIndexInput;
    OMX_VIDEO_PARAM_RVTYPE def;
    def.nPortIndex = portIndex;
    def.nVersion.nVersion = version;
    def.nWidth = width;
    def.nHeight = height;
    status_t err = mOMX->setParameter(mNode, OMX_IndexParamVideoRv, &def, sizeof(def));
    CHECK_EQ(err, (status_t)OK);
}

void OMXCodec::setVP6Format(int32_t flip) {
    OMX_U32 portIndex = mIsEncoder ? kPortIndexOutput : kPortIndexInput;
    OMX_VIDEO_PARAM_VP6TYPE def;
    def.nPortIndex = portIndex;
    def.nFlip = flip;
    status_t err = mOMX->setParameter(mNode, OMX_IndexParamVideoVp6, &def, sizeof(def));
    CHECK_EQ(err, (status_t)OK);
}

void OMXCodec::setVC1Format(int32_t width, int32_t height, VC1DecStrmFmt fmt) {
    OMX_U32 portIndex = mIsEncoder ? kPortIndexOutput : kPortIndexInput;
    OMX_VIDEO_PARAM_WMVTYPE def;
    def.nPortIndex = portIndex;
    def.nMaxFrameWidth = width;
    def.nMaxFrameHeight = height;
    def.nVC1DecStrmFmt = fmt;
    status_t err = mOMX->setParameter(mNode, OMX_IndexParamVideoWmv, &def, sizeof(def));
    CHECK_EQ(err, (status_t)OK);
}
/*a@nufront end*/
...
static size_t getFrameSize(
    ...
    switch (colorFormat) {
        ...
        case OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar:      /* a@nufront */
...
void OMXCodec::setVideoInputFormat(
    ...
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP8, mime)) { /*a@nufront*/
    ...
    LCHLOGD1("width = %d, height = %d", width, height);
    ...
    switch (compressionFormat) {
        ...
        case OMX_VIDEO_CodingVP8: /*a@nufront*/
        {
...
status_t OMXCodec::setupVP8EncoderParameters(const sp<MetaData>& meta) {
    ...
    vp8type.nDCTPartitions = OMX_FALSE;
...
status_t OMXCodec::setVideoOutputFormat(
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
        compressionFormat = OMX_VIDEO_CodingAVC;
            ...
            || !strcasecmp(MEDIA_MIMETYPE_VIDEO_FLV1, mime)) {
        compressionFormat = OMX_VIDEO_CodingMPEG4;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_H263, mime)) {
        compressionFormat = OMX_VIDEO_CodingH263;
        ...
        compressionFormat = OMX_VIDEO_CodingMPEG2;
            ...
            || !strcasecmp(MEDIA_MIMETYPE_VIDEO_RV40, mime)) {
...
#if 1
    {
        ...
        CHECK(format.eColorFormat == OMX_COLOR_FormatYUV420Planar
               ...
               || format.eColorFormat == OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar    /* a@nufront: nufront color format extended. */
               ...
               /*a@nufront end*/
        /*a@nufront start*/
        ZJFLOGD("will setParameter(OMX_IndexParamVideoPortFormat), mFlags is 0x%x, ComponentName is %s", mFlags, mComponentName);
        LCHLOGD1("%s: compressionFormat = %d, colorFormat = %d",mComponentName, (int) format.eCompressionFormat, (int) format.eColorFormat);

        if ((mFlags & kOutputColorFormat565) && !strcmp(mComponentName, "OMX.NU.Video.Decoder")) {
            ZJFLOGD("will set format.eColorFormat == OMX_COLOR_Format16bitRGB565");
            format.eColorFormat = OMX_COLOR_Format16bitRGB565;
        }
        /*a@nufront end*/
        /*a@nufront start: current operation is previewing image, notify decoder.*/
        if ((mFlags & kPreviewImage) && !strcmp(mComponentName, "OMX.NU.Video.Decoder")) {
            format.nPreviewImage = 1;
        }
        /*a@nufront end*/
        err = mOMX->setParameter(
                mNode, OMX_IndexParamVideoPortFormat,
...
uint32_t OMXCodec::initDvfsCnt = 0;
uint32_t OMXCodec::isDvfsFlag = 0;
char OMXCodec::dvfs_governor[DVFS_GOVERNOR_SIZE] = {'\0'};
OMXCodec::OMXCodec(
...
/*a@nufront: start*/
/*restore dvfs governor to the orginal settings*/
#ifdef GOVERNOR_CTRL
    if (OMXCodec::isDvfsFlag && !mIsEncoder) {
          if((mFrameWidth * mFrameHeight) >= 800*600) {
                OMXCodec::initDvfsCnt--;
                if(0 == OMXCodec::initDvfsCnt) {
                    OMXCodec::isDvfsFlag = 0;
                    property_set(DVFS_GOVERNOR_KEY, OMXCodec::dvfs_governor);
                    LOGV("Recovery Current DVFS Governor(%s)",OMXCodec::dvfs_governor);
                }
           }
    }
#endif
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow() {
    LCHLOGD1("allocateOutputBuffersFromNativeWindow");
    ...
    /*a@nufront start*/
#ifdef VPU_SUPPORT
    int halColorFormat = def.format.video.eColorFormat;
    if(!strcasecmp(mComponentName,"OMX.NU.Video.Decoder")){
        switch(def.format.video.eColorFormat){
            case OMX_COLOR_Format16bitRGB565:
                halColorFormat = HAL_PIXEL_FORMAT_RGB_565;
                break;
            case OMX_COLOR_Format32bitARGB8888:
                halColorFormat = HAL_PIXEL_FORMAT_RGBA_8888;
                break;
            case OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar:
                halColorFormat = HAL_PIXEL_FORMAT_YCbCr_420_SP;
                break;
            case OMX_COLOR_FormatYUV420SemiPlanar:/*nv12*/
                /*when our gpu support nv12,we should change this!*/
            default:
                halColorFormat = HAL_PIXEL_FORMAT_YCrCb_420_SP;/*nv21*/
                break;
        }
    }
    err = native_window_set_buffers_geometry(
            ...
            halColorFormat);
    LCHLOGD1("%s : halColorFormat=%d", mComponentName, halColorFormat);
#else
    /*a@nufront end*/
    err = native_window_set_buffers_geometry(
            mNativeWindow.get(),
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; i++) {
       ...
       if(!strcasecmp(mComponentName,"OMX.NU.Video.Decoder")){
           ...
           Rect bounds(def.format.video.nFrameWidth,def.format.video.nFrameHeight);
           CHECK_EQ(0,mapper.lock(buf->handle,GRALLOC_USAGE_SW_WRITE_OFTEN,bounds,&bufferId));
           YYLOGD("mapper.lock return bufferId=0x%p",bufferId);
       }
#endif
...
void OMXCodec::on_message(const omx_message &msg) {
    if (mState == ERROR) {
        /*m@nufront: msg need to be handled after error occurred. */
        LOGW("Dropping OMX message - we're in ERROR state.type = %d",msg.type);
        /* return; */
    }
...
void OMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        ...
        case OMX_EventError:
        {
            ...
            /*m@nufront: setting error in the Intermediastate will breaking down other operation. */
            if(false == isIntermediateState(mState))
                setState(ERROR);
            break;
        }
...
void OMXCodec::onCmdComplete(OMX_COMMANDTYPE cmd, OMX_U32 data) {
    switch (cmd) {
        ...
        case OMX_CommandFlush:
        {
            ...
            LCHLOGD1("%s : mState=%d, port=%d, status=%d", mComponentName, (int)mState, (int)portIndex, (int) mPortStatus[portIndex]);

            /* a@nufront start, only output can run here in reconfiguing status */
            if((RECONFIGURING == mState) && (kPortIndexOutput != portIndex)) {
                break;
            }

            /* return if port has beed disabled, video decoder may flush it 2 times */
            if((RECONFIGURING == mState) && (DISABLING == mPortStatus[portIndex])) {
                break;
            }
            /* a@nufront end */
...
bool OMXCodec::drainInputBuffer(BufferInfo *info) {
    ...
    if ((mCodecSpecificDataIndex < mCodecSpecificData.size()) && (strcmp(mComponentName, "OMX.NU.Audio.Decoder"))){
    ...
    for (;;) {
        ...
        if (mSeekTimeUs >= 0) {
            ...
            /*a@nufront: set seekflag to OMX*/
            flags |= OMX_BUFFERFLAG_SEEK;
...
/*a@nufront start: add to config nuaudio decoder*/
#ifdef WITH_APU_CODEC
status_t OMXCodec::setNuApuCodec(const sp<MetaData> &meta) {
    ZJFLOGD("function in.");
    int32_t err;
    int32_t numChannels;
    int32_t sampleRate;

    OMX_AUDIO_PARAM_NUAPUTYPE profileEntry, * profile = &profileEntry;
    ...
    CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
    CHECK(meta->findInt32(kKeySampleRate, &sampleRate));
    ZJFLOGD("mimeType is %s, numChannels is %d, sampleRate is %d", mMIME, numChannels, sampleRate);
    err = mOMX->getParameter(
            mNode, OMX_IndexParamAudioNuApu, profile, sizeof(profileEntry));
    if (err != OK) {
        ZJFLOGE("get apucodec OMX_IndexParamAudioNuApu error, err is %d", err);
        return err;
    }
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_MPEG, mMIME)) {
        ZJFLOGD("NUAPU Codec for MEDIA_MIMETYPE_AUDIO_MPEG");
        profile->eFormat = OMX_AUDIO_NUAPU_DEC_STREAM_MP3;
    } else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mMIME)) {
        ZJFLOGD("NUAPU Codec for MEDIA_MIMETYPE_AUDIO_AAC");
        profile->eFormat = OMX_AUDIO_NUAPU_DEC_STREAM_AAC;
    }

    err = mOMX->setParameter(
            mNode, OMX_IndexParamAudioNuApu, profile, sizeof(profileEntry));
    if (err != OK) {
        ZJFLOGE("set apucodec OMX_IndexParamAudioNuApu error, err is %d", err);
        return err;
    }
    ...
    if (err != OK) {
        ZJFLOGE("get apucodec OMX_IndexParamAudioPcm error, err is %d", err);
        return err;
    }

    params.nSamplingRate = sampleRate;
    params.nChannels = numChannels > 1 ? 2 : 1;

    err = mOMX->setParameter(
            mNode, OMX_IndexParamAudioPcm, &params, sizeof(params));

    if (err != OK) {
        ZJFLOGE("set apucodec OMX_IndexParamAudioPcm error, err is %d", err);
        return err;
    }
...
/*a@nufront start: add to config nuaudio decoder*/
status_t OMXCodec::setNuAudioDecoder(const sp<MetaData> &meta) {
    ZJFLOGD("function in.");
    int32_t codecCtxSize;
    int32_t codecCtxPtr;
    int32_t err;
    OMX_AUDIO_PARAM_NUAUDIOPROFILETYPE profileEntry, * profile = &profileEntry;
    memset(profile, 0, sizeof(profileEntry));
    mQuirks = mQuirks|kRequiresFlushBeforeShutdown;
    err = mOMX->getParameter(
            mNode, OMX_IndexParamAudioNuAudio, profile, sizeof(profileEntry));
    CHECK_EQ(err, (status_t)OK);

    /**
    if (meta->findInt32(kKeyFFmpegCodecPointerSize, &codecCtxSize) &&
            meta->findInt32(kKeyFFmpegCodecPointer, &codecCtxPtr)) {
        ZJFLOGD("codecCtxSize is %d, codecCtxPtr is 0x%x, profile size is %d", codecCtxSize, codecCtxPtr, sizeof(profile));
        memcpy(&profile, (void *)codecCtxPtr, codecCtxSize);
        CHECK_EQ(err, (status_t)OK);
    }
    **/
    int32_t metaTmp32Value;
    int64_t metaTmp64Value;
    char  metaTmpArry[500];
    const char * metaTmpCString = (const char *)metaTmpArry;

    if (meta->findInt32(kKeyCodecCtxCodecType, &metaTmp32Value)) {
        profile->codec_type = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxCodecID, &metaTmp32Value)) {
        profile->codec_id = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxCodecTag, &metaTmp32Value)) {
        profile->codec_tag = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxBitRate, &metaTmp32Value)) {
        profile->bit_rate = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxFlags, &metaTmp32Value)) {
        profile->flags = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxFlags2, &metaTmp32Value)) {
        profile->flags2 = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxProfile, &metaTmp32Value)) {
        profile->profile = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxTimeBaseNum, &metaTmp32Value)) {
        profile->time_base_num = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxTimeBaseDen, &metaTmp32Value)) {
        profile->time_base_den = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxExtraDataSize, &metaTmp32Value)) {
        profile->extradata_size = metaTmp32Value;
        if(meta->findInt32(kKeyCodecCtxExtraData, &metaTmp32Value)) {
            profile->extradata = (void *)metaTmp32Value;
        }
    }
    if (meta->findInt32(kKeyCodecCtxSampleFmt, &metaTmp32Value)) {
        profile->sample_fmt = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxSampleRate, &metaTmp32Value)) {
        profile->sample_rate = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxChannels, &metaTmp32Value)) {
        profile->channels = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxFrameSize, &metaTmp32Value)) {
        profile->frame_size = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxFrameNumber, &metaTmp32Value)) {
        profile->frame_number = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxBlockAlign, &metaTmp32Value)) {
        profile->block_align = metaTmp32Value;
    }
    if (meta->findInt64(kKeyCodecCtxChannelLayout, &metaTmp64Value)) {
        profile->channel_layout = metaTmp64Value;
    }
    if (meta->findInt32(kKeyCodecCtxTicksPerFrame, &metaTmp32Value)) {
        profile->ticks_per_frame = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxAudioServiceType, &metaTmp32Value)) {
        profile->audio_service_type = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxBitsPerCodedSample, &metaTmp32Value)) {
        profile->bits_per_coded_sample = metaTmp32Value;
    }
    if (meta->findInt32(kKeyCodecCtxBitsPerRawSample, &metaTmp32Value)) {
        profile->bits_per_raw_sample = metaTmp32Value;
    }
//    if (meta->findCString(kKeyCodecCtxCodecNameArry, &metaTmpCString)) {
//        strcpy(profile->codec_name, metaTmpCString);
//    }

    LCHLOGD1("mMIME = %s", mMIME);
    if((!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mMIME)) && (!strcmp(mComponentName, "OMX.NU.Audio.Decoder")))
    {
        FFmpegUtils::SetAudioProfile(profile, (const char* ) mMIME);
        FFmpegUtils::DumpAudioProfile(profile);

        if(meta->findInt32(kKeyChannelCount, &metaTmp32Value))
        {
            profile->channels = metaTmp32Value;
        }

        if(meta->findInt32(kKeySampleRate, &metaTmp32Value))
        {
            profile->sample_rate = metaTmp32Value;
        }

        if(mCodecSpecificData.size() > 0x0)
        {
            const CodecSpecificData *specific = mCodecSpecificData[0];
            profile->extradata_size = specific->mSize;
            profile->extradata = (void* ) specific->mData;
        }
    }
    ...
    if (err != OK) {
        ZJFLOGE("set NuAudioDec codecCtx error, err is %d", err);
        return err;
    }
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    while (mState != ERROR && !mNoMoreOutputData && mFilledBuffers.empty()) {
        ...
        /*a@nufront end*/
        if ((err = waitForBufferFilled_l()) != OK) {
            if(countBuffersWeOwn(mPortBuffers[kPortIndexOutput])==0 && countBuffersWeOwn(mPortBuffers[kPortIndexInput])==0)
                return err;
        }
    }
...
static const char *colorFormatString(OMX_COLOR_FORMATTYPE type) {
    ...
    /* a@nufront begin */
    else if (type == OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar) {
        return "OMX_NUFRONT_COLOR_FormatYUV420SemiPlanar";
    }
    /* a@nufront end */
...
status_t OMXCodec::initNativeWindow() {
    ...
    LCHLOGD1("mNativeWindow=%d", LCHISEXIST(mNativeWindow));
...
void OMXCodec::initOutputFormat(const sp<MetaData> &inputFormat) {
    ...
    switch (def.eDomain) {
        ...
        case OMX_PortDomainVideo:
        {
            ...
            } else if (video_def->eCompressionFormat == OMX_VIDEO_CodingVP8) {/*a@nufront*/
                mOutputFormat->setCString(
            ...
            if (video_def->nDisplayWidth != 0 && video_def->nDisplayHeight != 0) {
                mOutputFormat->setInt32(kKeyDisplayWidth, video_def->nDisplayWidth);
                mOutputFormat->setInt32(kKeyDisplayHeight, video_def->nDisplayHeight);
            }
            /*a@nufront start:
              ColorFormat is OMX_COLOR_FormatYUV420SemiPlanar if OMX output buffer is YUV data,
              ColorFormat is OMX_COLOR_Format16bitRGB565 if OMX output buffer is RGB data.*/
            /*a@nufront end*/
            LOGD("nufront: video_def->eColorFormat is %d", video_def->eColorFormat);
            mOutputFormat->setInt32(kKeyColorFormat, video_def->eColorFormat);

            /*a@nufront: ONLY CODEC_ID_VP6 needs flip vertically.*/
            if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP6, mMIME)) {
                mOutputFormat->setInt32(kKeyFlip, FLIP_V);
            }
...
/*a@nufront: start*/
#ifdef GOVERNOR_CTRL
/*CPU Change governors based on DVFS*/
int32_t OMXCodec::readGovernor() {
    FILE *fp = NULL;
    int i = 0;
    fp = fopen(DVFS_GOVERNOR_FILE,"r");
    if(NULL == fp) {
        LOGE("ERR(%s), Open %s Failed.ERR FOR:%s(%d)", __func__,
                                        DVFS_GOVERNOR_FILE, strerror(errno),errno);
        /* fclose(fp); */
        return -1;
    }
    /*read a line from governor file*/
    fgets(dvfs_governor, DVFS_GOVERNOR_SIZE, fp);
    if(strlen(dvfs_governor) < 1) {
        LOGE("ERR(%s),Failed to get DVFS GOVERNOR  FOR:%s(%d)", __func__,
                                                            strerror(errno),errno);
        fclose(fp);
        return -1;
    }
    dvfs_governor[strlen(dvfs_governor)-1] = '\0';
    LOGV("INFO(%s)->The current camera HAL governor is: %s", __func__, dvfs_governor);
    fclose(fp);
    return 0;
}
#endif
