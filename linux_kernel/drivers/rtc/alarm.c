...
#ifdef CONFIG_RTC_POWER_OFF_ALARM
static void rtc_print_time(struct rtc_time *ptm)
{
...
static void alarm_shutdown(struct platform_device *pdev)
{
	...
	wakeup_queue = &alarms[ANDROID_ALARM_RTC_WAKEUP];
	...
	if (wakeup_queue) {
		...
		rtc_time_to_tm(rtc_alarm_time, &rtc_alarm.time);
		dev_info(&pdev->dev, "alarm time:%ld\n", rtc_alarm_time);
		rtc_print_time(&rtc_alarm.time);
