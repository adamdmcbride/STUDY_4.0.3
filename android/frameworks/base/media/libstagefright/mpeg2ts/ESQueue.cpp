...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAAC() {
    ...
    while (offset + 7 <= mBuffer->size()) {
        /*a@nufront begin*/
        if (!IsSeeminglyValidADTSHeader(((const uint8_t* ) (mBuffer->data() + offset)), (mBuffer->size() - offset))) {
            LCHLOGD1("invalid AAC header, size=%d, offset=%d", (mBuffer->size() - offset), offset);
            offset++;
            continue;
        }
        /*a@nufront end*/
        ...
        /*m@nufront begin*/
        // adts_fixed_header
        /* when occurs invalid AAC frame, discard it and continue to decode the next frame, instead of crashing system directly*/
        //CHECK_EQ(bits.getBits(12), 0xfffu);
        if(bits.getBits(12) != 0xfffu) {
            LOGE("invalid AAC frame, discard it");
            return NULL;
        }
        /*m@nufront end*/
