...
int Volume::formatVol() {
    ...
    n = getDeviceNodes((dev_t *) &deviceNodes, 10);
    if (!n) {
    ...
    sprintf(devicePath, "/dev/block/vold/%d:%d",
            MAJOR(deviceNodes[mPartIdx-1]), MINOR(deviceNodes[mPartIdx-1]));
...
int Volume::mountVol() {
    ...
    // @TEI modified 4 to 10
    n = getDeviceNodes((dev_t *) &deviceNodes, /*4*/10);
    if (!n) {
        SLOGE("Failed to get device nodes (%s)\n", strerror(errno));
        return -1;
    }
    ...
    if (primaryStorage &&
        ...
        */
        ...
        if (createDeviceNode(nodepath, new_major, new_minor)) {
         ...
         SLOGD("updateDeviceInfo (%s), new_major, new_minor\n", nodepath, new_major, new_minor );
    ...
    for (i = 0; i < n; i++) {
        ...
        //@LWC add, fix bug 0003135 avoid sdcard not mount
        bool isFatFs = false;
        bool isNtfsFS = false;
...
//@LWC let adb push have Permission to sdcardEx and usbdisk
/*
        int gid;
...
*/
            ...
            //cuixiyuan 2012-06-11
            //fix bug 1496: Open the sdcard permissions, system user can read/write it.
        if (Fat::doMount(devicePath, "/mnt/secure/staging", false, false, false,AID_SYSTEM, AID_SDCARD_RW, 0002, true)) {
        ...
        if ( !isFatFs ) {
           if (Ntfs::doMount(devicePath, "/mnt/secure/staging",true)) {

		if(Ntfs::fix(devicePath)){
...
int Volume::initializeMbr(const char *deviceNode) {
    ...
    dinfo.sect_size = 512;
   // dinfo.skip_lba = 2048;
    dinfo.skip_lba = 512; //TEL@--for issue 1203
    dinfo.num_lba = 0;
    dinfo.num_parts = 1;
    ...
    int rc = apply_disk_config(&dinfo, 1);//TEL@-- for issue 1203
