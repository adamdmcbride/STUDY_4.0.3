...
rinse_repeat:
        ...
        if (mPlaylist->isComplete()) {
            ...
            while (index < mPlaylist->size()) {
                ...
                if (mSeekTimeUs < segmentStartUs + itemDurationUs) {
                    LCHLOGV1("mSeekTimeUs=%f s, %d/%d", (mSeekTimeUs/1000000.0), index, mPlaylist->size());
                    break;
                }
    ...
    /* a@nufront start */
    bool ptsDiscontinuity = false;
    if (itemMeta->findInt32("ptsdiscontinuity", &val) && val != 0) {
        ptsDiscontinuity = true;
        explicitDiscontinuity = true;
    }
    /* a@nufront end */

    LCHLOGD1("mSeqNumber=%d, first=%d, dis=%d, ptsdis=%d url=%s", mSeqNumber,firstSeqNumberInPlaylist, (int) explicitDiscontinuity, (int) ptsDiscontinuity, uri.c_str());
    ...
    if (buffer->size() == 0 || buffer->data()[0] != 0x47) {
        ...
        /* a@nufront start */
        if((mBandwidthItems.size() <= 0x1 ) && (0x0 == buffer->size()) &&
            ((mSeqNumber - firstSeqNumberInPlaylist) < ((int32_t) mPlaylist->size())))
        {
            LCHLOGD1("skip..., len=%d, idx=%d, listsize=%d, bandsize=%d",
                buffer->size(), bandwidthIndex, mPlaylist->size(), mBandwidthItems.size());

            /*if(mHTTPDataSource != NULL) {
                off64_t size = 0x0;
                mHTTPDataSource->getSize(&size);
                if(size <= 0x0) {
                    seekDiscontinuity = true;
                }
            }*/
...
skip:
 /* a@nufront end */
    ...
    if (seekDiscontinuity || explicitDiscontinuity || bandwidthChanged) {
        ...
        LOGI("queueing discontinuity (seek=%d, explicit=%d, bandwidthChanged=%d, ptsDiscontinuity=%d)",
             seekDiscontinuity, explicitDiscontinuity, bandwidthChanged, ptsDiscontinuity);
        ...
        /* a@nufront start */
        int64_t durationUs = 0x0;
        size_t index = 0x0;

        for(int32_t i = 0x0; i <= (mSeqNumber - firstSeqNumberInPlaylist); i++) {
            sp<AMessage> itemMeta;
            CHECK(mPlaylist->itemAt(i, NULL, &itemMeta));

            if (itemMeta->findInt32("discontinuity", &val) && val != 0) {
                index = (size_t) i;
            } else if (itemMeta->findInt32("ptsdiscontinuity", &val) && val != 0) {
                index = (size_t) i;
            }
        }

        for(size_t i = 0x0; i < index; i++) {
            int64_t itemDurationUs = 0x0;
            sp<AMessage> itemMeta;
            CHECK(mPlaylist->itemAt(i, NULL, &itemMeta));
            CHECK(itemMeta->findInt64("durationUs", &itemDurationUs));
        ...
        tmp->data()[2] = (ptsDiscontinuity) ? 0x3 : 0x2;
        memcpy(&(tmp->data()[3]), (void*) &durationUs, sizeof(durationUs));
        LCHLOGD1("No=%d, idx=%d, preroll=%f", (mSeqNumber - firstSeqNumberInPlaylist), index, (durationUs/1000000.0));
        /* a@nufront end */
...
void LiveSession::onSeek(const sp<AMessage> &msg) {
    ...
    LCHLOGV1("time=%lld (%f s)", timeUs, (timeUs/1000000.0));
...
status_t LiveSession::getDuration(int64_t *durationUs) {
    ...
    *durationUs = mDurationUs;
    LCHLOGD1("mDurationUs=%f", (mDurationUs/1000000.0));
