    ...
    private static final class VideoPlayer {
            ...
            if (mHTML5VideoView != null
                ...
                if (layer != 0 && surfTexture != null && currentVideoLayerId != -1) {
                    ...
                    if (playerState >= HTML5VideoView.STATE_PREPARED
                            && !foundInTree) {
                        /* d@nufront start: [workaround]
                         *
                         * Browser crash will reproduce if doing as below steps:
                         * 1. set user agent to ipad
                         * 2. open any url on web site such as www.youku.com
                         * 3. play video with html5 video tag, here is ok
                         * 4. open any other url
                         * 5. play video with html5 video tag, here is unseccessful
                         * 6. open the third url
                         * 7. play video with html5 video tag, crash occurs
                         *
                         * reason: When doing the 4th step, baseLayer will change to new tab,
                         * because new video does not played yet, currentVideoLayerId is the old,
                         * foundInTree is false, and because mHTML5VideoView is singleton,
                         * the old video will pause and surfacetexture will be deleted,
                         * THIS is the cause why the issue occurs.
                         * I don't know why currentVideoLayerId is still not be refresh
                         * when doing the 5th step. The layer seems be messy.
                         * And doing the 7th step, layers in WebCore have the conflict that
                         * the count of their children is lager than they have in fact.
                         *
                         * affect: The affect of the modification seems none.
                         * 1. video pause will be directly called by WebCore,
                         * 2. SurfaceTexture is a java object, it is managed by JVM.
                         *
                         * It seems foundInTree does not need to be cared.
                         * setBaseLayer will be called by UI at every time if input stream
                         * has changed. So although foundInTree is false at the begining,
        ...
        public static void onPrepared() {
            ...
            if (mBaseLayer != 0) {
                /* d@nufront start: [workaround]
                 *
                 * The issue will be resoved.
                 * Browser will crash when playing video after fullScreenExited.
                 *
                 * reason: The layer has errors after fullScreenExited,
                 * the count of its children is larger than it has in fact.
                 * So Browser will crash if nativeSendSurfaceTexture.
                 *
                 * affect: The affect of the modification seems none.
                 * After mHTML5VideoView.start(), setBaseLayer will called by WebView,
