...
// @TEI workaround bug related with mmcblkNbootN
   if(  (strstr(devpath, "boot0")) || (strstr(devpath, "boot1")) ){
        SLOGW("@@@Skip volumes handled block event for '%s'", devpath);
        return;
   }
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    if(!strcmp(label, "/mnt/sdcard")){
       ...
       if (write(tfcard_fd, nodepath, strlen(nodepath)) < 0) {
           SLOGE("Unable to write to ums lunfile1 (%s)", strerror(errno));
           close(tfcard_fd);
           return -1;
       }
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    if(!strcmp(label, "/mnt/sdcard")){
      ...
      char tfcard_ch = 0;
      if (write(tfcard_fd, &tfcard_ch, 1) < 0) {
          SLOGE("Unable to write to ums lunfile1 (%s)", strerror(errno));
          close(tfcard_fd);
          return -1;
      }
